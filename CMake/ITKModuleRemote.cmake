...
function(_git_clone git_executable git_repository git_tag module_dir)
  execute_process(
    COMMAND
      "${git_executable}" clone "${git_repository}" "${module_dir}"
  ...
  execute_process(
    COMMAND
      "${git_executable}" checkout ${git_tag}
...
function(_git_update git_executable git_repository git_tag module_dir)
  ...
  if(NOT "${remote_origin_url}" STREQUAL "${git_repository}")
    message(
      WARNING
      "Remote URL changed from ${git_repository} to ${remote_origin_url}"
    )
    execute_process(
      COMMAND
        "${git_executable}" remote rename origin old_origin
      WORKING_DIRECTORY "${module_dir}"
      RESULT_VARIABLE error_code
    ...
    execute_process(
      COMMAND
        "${git_executable}" remote add origin "${git_repository}"
      WORKING_DIRECTORY "${module_dir}"
      RESULT_VARIABLE error_code
      OUTPUT_VARIABLE ignored
      OUTPUT_STRIP_TRAILING_WHITESPACE
  ...
  execute_process(
    COMMAND
      "${git_executable}" rev-parse --verify "${git_tag}"
    WORKING_DIRECTORY "${module_dir}"
    RESULT_VARIABLE error_code
  ...
  if(NOT ("${tag_hash}" STREQUAL "${head_hash}"))
    execute_process(
      COMMAND
        "${git_executable}" fetch "${git_repository}"
      WORKING_DIRECTORY "${module_dir}"
      RESULT_VARIABLE error_code
    )
    if(error_code)
...
function(_fetch_with_git git_executable git_repository git_tag module_dir)
  if("${git_tag}" STREQUAL "" OR "${git_repository}" STREQUAL "")
    message(
      FATAL_ERROR
      "Tag or repository for git checkout should not be empty."
    )
  endif()
  ...
  if(NOT EXISTS "${module_dir}")
    ...
    message(
      STATUS
      " The remote module: ${git_repository} is cloned into the directory ${module_dir}"
    )
...
function(itk_fetch_module _name _description)
  ...
  set(
    Module_${_name}_REMOTE_COMPLIANCE_LEVEL
    ${MODULE_COMPLIANCE_LEVEL}
    CACHE INTERNAL
    "Variable to indicate the Module_${_name} compliance level"
  )

  if(NOT DEFINED Module_${_name})
    option(
      Module_${_name}
      "(Remote-${MODULE_COMPLIANCE_LEVEL}) ${_description}"
      OFF
    )
  ...
  if(Fetch_${_name})
    message(
      WARNING
      "Fetch_${_name} is deprecated, please use Module_${_name} to download and enable the remote module."
    )
    set(
      Module_${_name}
      ON
  ...
  if(Module_${_name})
    ...
    if(
      ...
      message(
        STATUS
        "NOTE: Using override 'Module_${_name}_GIT_TAG=${REMOTE_GIT_TAG}'\n"
    ...
    set(
      ...
      "${REMOTE_GIT_TAG}"
      CACHE STRING
      "Override default GIT_TAG value for remote module ${_name}"
    )
