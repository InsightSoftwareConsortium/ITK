...
static int
function_call(std::unique_ptr<A> a)
{
  return a.get() ? 1 : 0;
}
...
int
test_memory_main(int /*argc*/, char * /*argv*/[])
{
  int status = 0;
  ...
  // Keep everything in a subscope so we can detect leaks.
  {
    ...
    before = pa0.get();
    pa0.reset(new B());
    ASSERT(pa0.get() && pa0.get() != before, "auto_ptr does not hold a new object after reset(new B())");
    ...
    ASSERT(pa3.get(), "auto_ptr does not hold an object after assignment from factory function");
  }
