...
static void
test_if_bool_defined(bool)
{}

static void
test_if_int_defined(int)
{}
...
int
test_limits_main(int /*argc*/, char * /*argv*/[])
{
  ...
  {
    TEST("dinf", std::numeric_limits<double>::infinity() > std::numeric_limits<double>::max(), true);
  }
  TEST("dmin", std::numeric_limits<double>::min() < 1e-307 && std::numeric_limits<double>::min() > 0, true);
  TEST("deps", std::numeric_limits<double>::epsilon() < 1e-12 && std::numeric_limits<double>::epsilon() > 0, true);
  TEST("rnder", std::numeric_limits<double>::round_error() <= 1.0, true);
  TEST("fmax", std::numeric_limits<float>::max() > 1e38f, true);
  if (std::numeric_limits<float>::has_infinity)
  {
    TEST("finf", std::numeric_limits<float>::infinity() > std::numeric_limits<float>::max(), true);
  }
  TEST("fmin", std::numeric_limits<float>::min() < 1e-37f && std::numeric_limits<float>::min() > 0, true);
  TEST("feps", std::numeric_limits<float>::epsilon() < 1e-6f && std::numeric_limits<float>::epsilon() > 0, true);
