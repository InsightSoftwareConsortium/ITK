...
# baseline: a clean ``import itk`` on a current Linux/macOS dev box
# completes in well under a second of cumulative -X importtime cost
# and adds roughly 30-50 entries to sys.modules (itk + itk.<Module>
# synthetic submodules + stdlib trampolines).  The ceiling is set
# generously so slow CI runners do not flap; tighten if the
# headroom shrinks.
MAX_CUMULATIVE_IMPORT_MS = 5000.0
MAX_SYS_MODULES_AFTER_IMPORT_ITK = 400
