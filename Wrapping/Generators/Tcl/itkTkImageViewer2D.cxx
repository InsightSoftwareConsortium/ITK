...
TkImageViewer2D::Draw()
{
  ...
  // Setup the size
  Tk_PhotoHandle photo = Tk_FindPhoto(m_Interpreter, const_cast<char *>(m_ImageName.c_str()));
  Tk_PhotoSetSize(photo, width, height);

  std::ostringstream command;
  command << m_CanvasName.c_str() << " configure -scrollregion \"1 1 " << width << " " << height << "\"";
  ...
  unsigned char * buffer = reinterpret_cast<unsigned char *>(image->GetBufferPointer());
...
#else
  Tk_PhotoPutBlock(photo, &block, 0, 0, size[0], size[1], TK_PHOTO_COMPOSITE_SET);
#endif
}
