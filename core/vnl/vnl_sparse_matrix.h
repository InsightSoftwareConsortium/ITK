...
public:
  ...
  vnl_sparse_matrix_pair(const unsigned int & a, const T & b)
    : first(a)
    , second(b)
  {}
  ...
  operator=(const vnl_sparse_matrix_pair & o)
  {
    if (&o != this)
    {
      first = o.first;
  ...
  {
    bool
    operator()(const vnl_sparse_matrix_pair & p1, const vnl_sparse_matrix_pair & p2)
    {
...
public:
  ...
  vnl_sparse_matrix<T> &
  operator=(const vnl_sparse_matrix<T> & rhs);

  //: Multiply this*rhs, where rhs is a vector.
  void
  mult(const vnl_vector<T> & rhs, vnl_vector<T> & result) const;

  //: Multiply this*p, a fortran order matrix.
  void
  mult(unsigned int n, unsigned int m, const T * p, T * q) const;

  //: Multiplies lhs*this, where lhs is a vector
  void
  pre_mult(const vnl_vector<T> & lhs, vnl_vector<T> & result) const;

  //: Get a reference to an entry in the matrix.
  T &
  operator()(unsigned int row, unsigned int column);
  ...
  vnl_sparse_matrix<T> &
  vcat(const vnl_sparse_matrix<T> & A);
  ...
  bool
  empty_row(unsigned int r) const
  {
  ...
  vnl_sparse_matrix &
  scale_row(unsigned int r, T scale);
  ...
  //: Comparison
  bool
  operator==(const vnl_sparse_matrix<T> & rhs) const;
  ...
  void
  add(const vnl_sparse_matrix<T> & rhs, vnl_sparse_matrix<T> & result) const;

  //: Subtract rhs from this.
  //  Deprecated for direct use: please use operator "-" instead.
  void
  subtract(const vnl_sparse_matrix<T> & rhs, vnl_sparse_matrix<T> & result) const;

  //: Multiply this*rhs, another sparse matrix.
  //  Deprecated for direct use: please use operator "*" instead.
  void
  mult(const vnl_sparse_matrix<T> & rhs, vnl_sparse_matrix<T> & result) const;
...
template <class T>
inline vnl_sparse_matrix<T>
operator*(const T & value, const vnl_sparse_matrix<T> & m)
{
  return m * value;
}
