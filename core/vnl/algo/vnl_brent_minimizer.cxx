...
// Use f_at_last_minimum() to get function evaluation at the returned minima.
double
vnl_brent_minimizer::minimize_given_bounds_and_one_f(double ax, double bx, double cx, double fb)
{
  ...
  while (std::fabs(x - m) > (tol2 - 0.5 * (cx - ax)))
  {
    ...
    if (std::fabs(e) > tol)
    {
      ...
      if (q > 0)
        ...
        q *= -1.0; // q always positive
      r = e;
      e = d;
    }

    if (std::fabs(p) < std::fabs(0.5 * q * r) && p > (q * (ax - x)) && p < (q * (cx - x))) // So that ax<x+p/q<cx
    {
...
// Use f_at_last_minimum() to get function evaluation at the returned minima.
double
vnl_brent_minimizer::minimize_given_bounds_and_all_f(double ax, double bx, double cx, double fa, double fb, double fc)
{
  // Check that the bracket is valid
  assert(ax < bx);
  assert(bx < cx);
...
double
vnl_brent_minimizer::minimize(double x)
{
  double ax = x - 1.0;
  double cx = x + 1.0;
  double fa = NAN;
