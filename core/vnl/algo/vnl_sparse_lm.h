...
protected:
  ...
  void
  init(vnl_sparse_lst_sqr_function * f);
...
private:
  ...
  bool
  check_vector_sizes(const vnl_vector<double> & a, const vnl_vector<double> & b, const vnl_vector<double> & c);
  ...
  vnl_vector<double>
  extract_diagonal() const;

  //: set the vector on the diagonal of Jt J
  void
  set_diagonal(const vnl_vector<double> & diag);
  ...
  //: compute Z and Sa
  void
  compute_Z_Sa(vnl_matrix<double> & Sa);

  //: compute Ma
  void
  compute_Ma(const vnl_matrix<double> & H);
  ...
  //: solve for dc
  void
  solve_dc(vnl_vector<double> & dc);

  //: compute sea using ea, Z, dc, Y, and eb
  void
  compute_sea(const vnl_vector<double> & dc, vnl_vector<double> & sea);

  //: compute Sa and sea
  // only used when size_c_ == 0
  void
  compute_Sa_sea(vnl_matrix<double> & Sa, vnl_vector<double> & sea);

  //: back solve to find db using da and dc
  void
  backsolve_db(const vnl_vector<double> & da, const vnl_vector<double> & dc, vnl_vector<double> & db);
