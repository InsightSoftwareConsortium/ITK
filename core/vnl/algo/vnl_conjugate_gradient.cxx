...
void
vnl_conjugate_gradient::init(vnl_cost_function & f)
{
  f_ = &f;
  num_iterations_ = 0;
  num_evaluations_ = 0;
...
void
vnl_conjugate_gradient::preconditioner_(double * out, double * in, void * userdata)
{
...
vnl_conjugate_gradient::minimize(vnl_vector<double> & x)
{
  ...
  if (error_code > 0)
  {
    ...
    if (verbose_)
    {
      ...
      {
        case 1:
          std::cout << "UNABLE TO OBTAIN DESCENT DIRECTION\n";
          break;
        case 2:
          std::cout << "THE FUNCTION DECREASES WITH NO MINIMUM\n";
          break;
        case 3:
          std::cout << "PRECONDITIONER NOT POSITIVE DEFINITE\n";
          break;
        case 4:
          std::cout << "UNABLE TO SATISFY ARMIJO CONDITION\n";
          break;
        default:
...
void
vnl_conjugate_gradient::diagnose_outcome(std::ostream & os) const
{
  os << "vnl_conjugate_gradient: " << num_iterations_ << " iterations, " << num_evaluations_
     << " evaluations. Cost function reported error" << f_->reported_error(start_error_) << '/'
     << f_->reported_error(end_error_) << " . Final step size = " << final_step_size_ << std::endl;
}

void
vnl_conjugate_gradient::diagnose_outcome() const
{
