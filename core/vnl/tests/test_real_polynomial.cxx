...
test_real_polynomial()
{
  ...
  const vnl_real_polynomial f3 = f1 + f2;
  f3.print(std::cout);
  std::cout << std::endl;
  ...
  const vnl_real_polynomial f4 = f1 - f2;
  f4.print(std::cout);
  std::cout << std::endl;
  TEST("f1-f2=f4", f1.evaluate(2.5) - f2.evaluate(2.5), f4.evaluate(2.5));

  const vnl_real_polynomial f5 = f1 * f2;
  f5.print(std::cout);
  std::cout << std::endl;
  ...
  const vnl_real_polynomial f1d = f1.derivative();
  f1d.print(std::cout);
  std::cout << std::endl;
  const vnl_real_polynomial f2d = f2.derivative();
  f2d.print(std::cout);
  std::cout << std::endl;

  TEST("Derivative", f5.derivative(), f1d * f2 + f2d * f1);

  const vnl_real_polynomial f5p = (f1d * f2 + f2d * f1).primitive();
  f5p.print(std::cout);
  std::cout << std::endl;
  ...
  const vnl_vector<double> v(v_data, 3);
