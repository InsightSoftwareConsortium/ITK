...
VNL_EXPORT void
vnl_na_extract(std::istream & is, double & x);
...
// character sequence \code NA.
VNL_EXPORT void
vnl_na_insert(std::ostream & is, double x);

//: Read a floating point number or "NA" from a stream.
// Should behave exactly like a>>x, if the extraction operator was aware of the
// character sequence \code NA.
VNL_EXPORT void
vnl_na_extract(std::istream & is, float & x);
...
// character sequence \code NA.
VNL_EXPORT void
vnl_na_insert(std::ostream & is, float x);
...
{
  const T & x_;
  vnl_na_stream_const_t(const T & x)
    : x_(x)
  {}
};
...
//: Wrap a double or float to handle streaming NA.
template <class T>
inline vnl_na_stream_const_t<T>
vnl_na_stream(const T & x)
{
...
template <class T>
inline std::ostream &
operator<<(std::ostream & os, const vnl_na_stream_t<T> & ns)
{
...
template <class T>
inline std::ostream &
operator<<(std::ostream & os, const vnl_na_stream_const_t<T> & ns)
{
  vnl_na_insert(os, ns.x_);
  return os;
}
...
template <class T>
inline std::istream &
operator>>(std::istream & is, const vnl_na_stream_t<T> & ns)
{
