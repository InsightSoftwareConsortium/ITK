...
template <class T>
inline VNL_EXPORT double
vnl_beta(T x, T y)
{
  return std::exp(vnl_log_gamma(x) + vnl_log_gamma(y) - vnl_log_gamma(x + y));
}
...
vnl_beta(T x, T y)
{
  return (vnl_gamma(x) * vnl_gamma(y)) / vnl_gamma(x + y);
}
#endif
...
template <class T>
inline VNL_EXPORT double
vnl_log_beta(T x, T y)
{
  return vnl_log_gamma(x) + vnl_log_gamma(y) - vnl_log_gamma(x + y);
}
