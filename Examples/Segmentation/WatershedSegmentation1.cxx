...
// prepared will greatly affect the quality of your result.  Typically, the
// best results are obtained by preprocessing the original image with an
// edge-preserving diffusion such as one of the anisotropic diffusion filters,
...
int main( int argc, char **argv )
{
  ...
  // using an anisotropic diffusion filter.  For this class of filters, the CFL
  // condition requires that the time step be no more than 0.25 for two-
  // dimensional images, and no more than 0.125 for three-dimensional images.
  // The number of iterations and the conductance term will be taken from the
  // command line. See Section \ref{sec:EdgePreservingSmoothingFilters} for
  // more information on the ITK anisotropic diffusion filters.
  //
  // Software Guide : EndLatex
  ...
  // take several parameters.  Here we allow only enabling or disabling of 
