...
// To support the re-mapping of the moving image intensity we declare an internal
// image type with a floating point pixel type and cast the input images to
...
// same homologous point on an object have the same intensity on both 
// the fixed and moving images to be registered. In this example, 
// we will preprocess the moving image to match the intensity between the
// images using the \code{HistogramMatchingImageFilter}. 
//
...
// Software Guide : BeginLatex
// 
// The registration algorithm is triggered by updating the filter. The filter output
// is the computed deformation field.
//
// Software Guide : EndLatex
// Software Guide : BeginCodeSnippet
  filter->Update();
...
// Software Guide : BeginLatex
//
// A \code{WarpImageFilter} can be used to warp the moving image with the output
// deformation field. Similar to the \code{ResampleImageFilter}, the \code{WarpImageFilter}
// requires the specification of the input image to be resampled, an input image interpolator,
// the output image spacing and origin.
...
// Software Guide : EndCodeSnippet
  ...
  // Software Guide : BeginLatex
  //
  // Let's execute this example using rat lung data in the previous example. The associated data files
  // can be found in \code{Insight/Examples/Data}:
  // \begin{itemize}
  ...
  // \caption{Checkerboard comparisons before and after demons-based deformable registration.}
  // \label{fig:DeformableRegistration2Output}
  // \end{figure}
  // 
  // The results of the demons-based deformable registration is presented in Figure 
  // \ref{fig:DeformableRegistration2Output}. The checkerboard comparision shows that the
  // algorithm was able to recover the misalignment due to expiration.
  //
  // Software Guide : EndLatex
