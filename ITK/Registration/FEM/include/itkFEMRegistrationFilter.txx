...
void FEMRegistrationFilter< TMovingImage, TFixedImage >::RunRegistration(void)
{
  ...
  if ( !m_DoMultiRes && m_Maxiters[m_CurrentLevel] > 0 )
    ...
    if ( !m_ReadMeshFile )
      ...
      {
      nzelts =
        ( ( 2 * numnodesperelt * ndofpernode * ndof > 25
            * ndof ) ? 2 * numnodesperelt * ndofpernode * ndof : 25 * ndof );
      }
...
//  typedef itk::DemonsImageToImageMetric<FixedImageType,MovingImageType>
// MetricType5;
                                                                    ...
                                                                    Solver & mySolver, ImageSizeType imagesize)
{
  ...
  for ( unsigned int i = 0; i < ImageDimension; i++ )
    ...
    ImageSizeV[i] = (double)imagesize[i] + 1;  //to make sure all elts are
                                               // inside the interpolation mesh
  ...
  if ( m_ReadMeshFile )
    ...
    if ( m )
      {
      m->E = this->GetElasticity(m_CurrentLevel);  // Young modulus -- used in
                                                   // the membrane ///
      }
...
template< class TMovingImage, class TFixedImage >
void FEMRegistrationFilter< TMovingImage, TFixedImage >::ApplyLoads(SolverType & mySolver,
                                                                    ImageSizeType ImgSz,
                                                                    double *scaling)
{
  //
  ...
  if ( m_UseLandmarks )
    ...
    if ( this->m_LandmarkArray.empty() )
      ...
      if ( f )
        ...
        try
          {
          mySolver.load.clear(); // NOTE: CLEARING ALL LOADS - LMS MUST BE
                                 // APPLIED FIRST
          mySolver.Read(f);
          }
        ...
        mySolver.load.clear(); // NOTE: CLEARING ALL LOADS - LMS MUST BE APPLIED
                               // FIRST
        }
      else
        {
...
// now scale the landmarks
  ...
  while ( node != nodes->end() && EdgeCounter < ImageDimension )
    ...
    if ( CornerCounter == ImageDimension ) // the node is located at a true
                                           // corner
      {
        ...
        for ( whichnode = 0; whichnode <= maxnode; whichnode++ )
          ...
          if ( EdgeFound )
            ...
            for ( unsigned int jj = 0; jj < ndofpernode; jj++ )
              ...
              l1 = LoadBC::New();
              // now we get the element from the node -- we assume we need fix
              // the dof only once
              // even if more than one element shares it.
              l1->m_element = ( *elt );  // Fixed bug TS 1/17/03 (
                                         // *((*node)->m_elements.begin()));
...
void FEMRegistrationFilter< TMovingImage, TFixedImage >::IterativeSolve(SolverType & mySolver)
{
  ...
  while ( !Done && iters < m_Maxiters[m_CurrentLevel] )
    ...
    // uncomment to write out every deformation SLOW due to interpolating vector
    // field everywhere.
...
//  std::cout << " r s t steps " << rstep << " " << sstep << " "<< tstep <<
// std::endl;
...
//        float interperror=0.0;
  ...
  // Insure that the values are exact at the nodes. They won't necessarily be
  // unless we use this code.
                                                                               ...
                                                                               bool onlywriteimages)
{
  ...
  if ( m_MinJacobian < thresh ) //FIXME
    ...
    if ( resize )
      {
      fullField = ExpandVectorField(expandFactors, m_Field); // this works -
                                                             // linear interp
                                                             // and expansion of
                                                             // vf
      }
    ...
    if ( m_UseLandmarks )
      ...
      if ( !m_LandmarkArray.empty() )
        ...
        for ( unsigned int lmind = 0; lmind < m_LandmarkArray.size(); lmind++ )
          {
          std::cout << " old source " << m_LandmarkArray[lmind]->GetSource() << " target "
                    << m_LandmarkArray[lmind]->GetTarget() << std::endl;

          // Convert the source to warped coords.
          m_LandmarkArray[lmind]->GetSource() = m_LandmarkArray[lmind]->GetSource()
                                                + ( dynamic_cast< LoadLandmark * >( &*mySolver.load.Find(lmind) )->
                                                    GetForce() );
          std::cout << " new source " << m_LandmarkArray[lmind]->GetSource() << " target "
                    << m_LandmarkArray[lmind]->GetTarget() << std::endl;
...
template< class TMovingImage, class TFixedImage >
void FEMRegistrationFilter< TMovingImage, TFixedImage >::FindBracketingTriplet(SolverType & mySolver,
                                                                               Float *a,
                                                                               Float *b,
                                                                               Float *c)
{
  ...
  Float cx = bx + Gold * ( bx - ax );  // first guess for c - the 3rd pt needed
                                       // to bracket the min
...
template< class TMovingImage, class TFixedImage >
Element::Float FEMRegistrationFilter< TMovingImage, TFixedImage >::GoldenSection(SolverType & mySolver,
                                                                                 Float tol,
...
template< class TMovingImage, class TFixedImage >
void FEMRegistrationFilter< TMovingImage, TFixedImage >::PrintSelf(std::ostream & os, Indent indent) const
{
