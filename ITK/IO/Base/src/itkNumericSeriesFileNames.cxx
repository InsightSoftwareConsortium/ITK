...
::GetFileNames()
{
  ...
  for ( SizeValueType i = m_StartIndex; i <= m_EndIndex; i += m_IncrementIndex )
    ...
    int nchars = snprintf(c, sizeof(c), m_SeriesFormat.c_str(), i);
    if(nchars <= -1)
      {
      // broken snprintf reports overflow with -1
      nchars = m_SeriesFormat.size() + sizeof(c); // enough room for path +
                                                  //absurdly long integer string.
      }
    int bufflen = nchars + 1;
    char *temp = new char[bufflen];
    int result = snprintf (temp, bufflen, m_SeriesFormat.c_str(), i);
