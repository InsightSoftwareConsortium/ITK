...
- CMake global and directory scoped properties such and incude directories and libraries have been replaced with target-specific properties.
...
The usage of `UseITK.cmake` is now depricated, and provides the legacy compatibility of setting global properties for include directory, CXX flags etc.
...
include_directories(${MyModule_SOURCE_DIR}/include)
...
code to register loaded IO and factory-enabled components from the modules specified in `find_package()`.
...
For backward compatibility, non-namespaced aliases are created with deprecation warnings. However, new code should
use the namespaced `ITK::` targets exclusively.
