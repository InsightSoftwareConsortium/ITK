...
The enumeration defined in `itk::SpatialOrientation` including `itk::SpatialOrientation::CoorindateTerms`,
...
of itself as an unambiguous enumeration, string and a matrix. It provides both a `PositveEnum` and a `NegativeEnum` for
...
reoriented_image = itk.orient_image_filter(
    ...
    desired_coordinate_orientation=itk.AnatomicalOrientation(
        itk.SpatialOrientationEnums.ValidCoordinateOrientations_ITK_COORDINATE_ORIENTATION_RAP
    ),
)
