...
#
#       Modified to use "string compare" instead of "string equal". This 
#       allows the script to run under Tcl 8.0
#
#
...
#
# Hack for tcl8.0 compatibility
#
proc ftp::StringEqual { string1 string2 } {
...
proc ftp::StateHandler {s {sock ""}} {
    ...
    if { $sock != "" } {
        ...
        if { $number > 0 } {
            ...
            while { [ftp::StringEqual $multi_line "-"] } {
                set number [gets $sock bufline]	
                if { $number > 0 } {
                    append buffer \n "$bufline"
    ...
    switch -exact -- $ftp(State) {
        ...
        type {
            if { [ftp::StringEqual $ftp(Type) "ascii"] } {
        ...
        nlist_open {
            switch -exact -- $rc {
                2 {
                    if { [ftp::StringEqual $ftp(Mode) "passive"] } {
...
#         }
    ...
    if { ![ftp::StringEqual "[Type $s]" "$old_type"] } {
        Type $s $old_type
    }
...
if { [ftp::StringEqual [uplevel "#0" {info commands tkcon}] "tkcon"] } {
