...
//    double maxerr = 1.9e-15;
  ...
  { 10-0, 20-0,  40-10},
    ...
    // 
    // Now exercise the option of UseRegion
    // 
    moments->SetUseRegionOfInterest( true );
    if( moments->GetUseRegionOfInterest() != true )
      {
      std::cerr << "Error in Set/Get UseRegionOfInterest " << std::endl;
      pass = false;
      }

    moments->SetUseRegionOfInterest( false );
    if( moments->GetUseRegionOfInterest() != false )
      {
      std::cerr << "Error in Set/Get UseRegionOfInterest " << std::endl;
      pass = false;
      }

    moments->UseRegionOfInterestOn();
    if( moments->GetUseRegionOfInterest() != true )
      {
      std::cerr << "Error in Set/Get UseRegionOfInterest " << std::endl;
      pass = false;
    ...
    moments->UseRegionOfInterestOff();
    if( moments->GetUseRegionOfInterest() != false )
      {
      std::cerr << "Error in Set/Get UseRegionOfInterest " << std::endl;
      pass = false;
    ...
    CalculatorType::PointType regionOfInterestCorner1;
    CalculatorType::PointType regionOfInterestCorner2;
    ...
    image->TransformIndexToPhysicalPoint( index1, regionOfInterestCorner1 );
    image->TransformIndexToPhysicalPoint( index2, regionOfInterestCorner2 );
    ...
    std::cout << "Region principal axes " << cpa << std::endl;
