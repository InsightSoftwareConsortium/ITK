...
int itkFixedCenterOfRotationAffineTransformTest(int, char *[])
{
    ...
    FCoRAffine2DType::MatrixType                   inverse2;
    ...
    point2 = id2->GetCenterOfRotation();
    std::cout << "Matrix from instantiating an identity transform:"
              << std::endl << matrix2;
    std::cout << "Vector from instantiating an identity transform:"
              << std::endl;
    std::cout << vector2 << std::endl;
    std::cout << "CenterOfRotation from instantiating an identity transform:"
              << std::endl << point2 << std::endl;
    ...
    aff2->SetCenterOfRotation( point2 );
    for (i = 0; i < 2; i++) 
      {
      for (j = 0; j < 2; j++)
        {
    ...
    FCoRAffine2DType::InputVectorType              trans;
    trans[0] = 1;
    trans[1] = 2;
    ...
    FCoRAffine2DType::InputVectorType              scale;
    scale[0] = .3;
    scale[1] = .2;
    aff2->Scale(scale);
    ...
    typedef itk::FixedCenterOfRotationAffineTransform<double,3> FCoRAffine3DType;
    FCoRAffine3DType::Pointer aff3 = FCoRAffine3DType::New();
    ...
    FCoRAffine3DType::Pointer jaff = FCoRAffine3DType::New();
    FCoRAffine3DType::MatrixType jaffMatrix = jaff->GetMatrix();
    FCoRAffine3DType::OffsetType jaffVector = jaff->GetOffset();

    FCoRAffine3DType::InputPointType jpoint;
    jpoint[0] = 5.0;
    jpoint[1] = 10.0;
    jpoint[2] = 15.0; 
    FCoRAffine3DType::JacobianType jaffJacobian = jaff->GetJacobian( jpoint );

    std::cout << "GetJacobian: " << std::endl;
    std::cout << jaffJacobian << std::endl;
    
    /* Test SetParameters */
    FCoRAffine3DType::Pointer paff = FCoRAffine3DType::New();
    FCoRAffine3DType::ParametersType parameters( paff->GetNumberOfParameters() );
