...
int FrontIteratorTest( int , char *[] )
{
  ...
  for( FrontIterator it = mesh->BeginFront();
    ...
    PointIdentifier org  = edge->GetOrg();
    PointIdentifier dest = edge->GetDest();

    if( !visitedSet.count( org ) )
      {
      visitedSet.insert( org );
      }
    if( !visitedSet.count( dest ) )
      {
