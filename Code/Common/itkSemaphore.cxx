...
void Semaphore::Initialize (int value)
{
  ...
  // by default the semaphre created has value 1
  Semaphore::unix_semaphore_key++;
  Semaphore::m_Mutex.Unlock();
  
  if (m_Sema == -1)
    {
      itkExceptionMacro( << "Call UnixIpcSemaphoreCreate failed. " );
    }
  ...
  m_Sema = usnewsema (MultiThreader::GetThreadArena(), value);
  if (!m_Sema)
    {
...
#ifdef ITK_USE_WIN32_THREADS
  m_Sema = static_cast<unsigned int>( CreateSemaphore( 0, value, 0x7FFFFFFF, (void *)0) );
  if (!m_Sema)
    {
...
#ifdef ITK_USE_WIN32_THREADS
  // SUYASH:  What is HANDLE?  Where is it declared?
...
#ifdef ITK_USE_WIN32_THREADS
  if (WaitForSingleObject((HANDLE)m_Sema, INIFITE) == WAIT_FAILED)
    {
...
#endif
#ifdef ITK_USE_PTHREADS
  if ((m_Sema != 0) && (sem_destroy(&m_Sema) != 0))
    {
