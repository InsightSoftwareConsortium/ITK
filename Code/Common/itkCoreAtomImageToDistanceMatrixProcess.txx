...
::CoreAtomImageToDistanceMatrixProcess()
{
  ...
  output =
    static_cast< typename CoreAtomImageToDistanceMatrixProcess::DistanceMatrixType * >( this->MakeOutput(0).GetPointer() );
...
::GenerateData()
{
  ...
  for ( bloxIt.GoToBegin(); !bloxIt.IsAtEnd(); ++bloxIt )
    ...
    for ( bloxIt2.GoToBegin(); !bloxIt2.IsAtEnd(); ++bloxIt2 )
      ...
      distance = vcl_sqrt( vcl_pow( (double)DistanceVector[0], 2.0 ) + vcl_pow( (double)DistanceVector[1],
                                                                                2.0 )
                           + vcl_pow( (double)DistanceVector[2], 2.0 ) );
