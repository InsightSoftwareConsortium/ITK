...
template<class TInputImage, class TOutputImage>
RegionGrowKLM<TInputImage,TOutputImage>::LabelImagePointer
RegionGrowKLM<TInputImage,TOutputImage>
::GetLabelledImage()
{
  //--------------------------------------------------------------------
...
{ 
  ...
  //----------------------------------------------------------------------
  m_pRegions = new RGRegionKLM<TInputImage,TOutputImage> [m_NumRegions];
  ...
  m_pBorders = new RGBorderKLM<TInputImage,TOutputImage> [m_nBorders];
  ...
  */
  ...
  for ( int r = nRowSquareBlocks - 1; r >= 1; r-- ) 
  {
    for ( int c = 0; c < nColSquareBlocks; c++ ) 
    {
      ...
      RGRegionKLM<TInputImage,TOutputImage> 
  ...
  m_pBordersDynPtrs = new RGBorderKLMPtr [m_nBorders];
  ...
  std::sort(m_pBordersDynPtrs, 
           (m_pBordersDynPtrs+m_nBorders), 
           std::greater < DynamicBorderArrayKLM<BorderType> >());
   
  m_pBordersCandidateDynPtr 
    = m_pBordersDynPtrs + m_nBorders - 1;
...
::merge_regions()
{
  ...
  RGRegionKLM<TInputImage,TOutputImage> *pRegion1;

  // Second region associated with the candidate border
  RGRegionKLM<TInputImage,TOutputImage> *pRegion2;
...
void
RegionGrowKLM<TInputImage,TOutputImage>
::union_borders(RGRegionKLM<TInputImage,TOutputImage> *pnewRegion,
                        RGRegionKLM<TInputImage,TOutputImage> *poldRegion)
{

  RGRegionKLM<TInputImage,TOutputImage> *ptmpRegion;  

  typedef 
    std::vector< RGBorderKLM<TInputImage,TOutputImage>* > RegionBorderVecT;
