  ...
  operator++()
  {
    if (++m_Offset >= m_SpanEndOffset)
      ...
      typename ImageIterator<TImage>::IndexType
        ind = m_Image->ComputeIndex( static_cast<typename OffsetValueType>(m_Offset) );
      ...
      done = (++ind[0] == startIndex[0] + static_cast<typename IndexValueType>(size[0]));
      for (unsigned int i=1; done && i < ImageIteratorDimension; i++)
        {
        done = (ind[i] == startIndex[i] + static_cast<typename IndexValueType>(size[i]) - 1);
        }
      ...
      if (!done)
        ...
        while ( ( dim+1 < ImageIteratorDimension )
          && (ind[dim] > startIndex[dim] +  static_cast<typename IndexValueType>(size[dim]) - 1) )
          {
  ...
  Self & operator--()
  {
    if (--m_Offset < m_SpanBeginOffset)
      ...
      ImageIterator<TImage>::IndexType
        ind = m_Image->ComputeIndex( static_cast<typename IndexValueType>(m_Offset) );
