...
{
  Neighborhood Ans;
  Neighborhood::ConstIterator in;
  Neighborhood::Iterator out;
  
  Ans.SetRadius(this->GetRadius());
  for (in = this->Begin(), out = Ans.End()-1; in < this->End(); ++in, --out)
    {
      // Mirror the whole pixel, not just the scalar part.
      *out = *in;
    }
