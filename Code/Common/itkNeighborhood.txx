...
Neighborhood(const Self& other)
{
  ...
  for (unsigned i = 0; i < VDimension; i++)
    { m_StrideTable[i] = other.m_StrideTable[i]; }
...
::GetSlice(unsigned int d) const
{
  ...
  return std::slice(static_cast<std::size_t>(n),
                    static_cast<std::size_t>(this->GetSize()[d]),
                    static_cast<std::size_t>(this->GetStride(d)) );
}
