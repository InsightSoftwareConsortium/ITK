...
C
     ...
     *   MAXOP, MAXJ, IND(1), IERR
      REAL VEC(NMVEC,1), VAL(NMVAL,1), WORK(1)
      EXTERNAL OP, IOVECT
C
...
C
C SNLASO FINDS A FEW EIGENVALUES AND EIGENVECTORS AT EITHER END OF
C THE SPECTRUM OF A LARGE SPARSE SYMMETRIC MATRIX.  THE SUBROUTINE
C SNLASO IS PRIMARILY A DRIVER FOR SUBROUTINE SNWLA WHICH IMPLEMENTS
C THE LANCZOS ALGORITHM WITH SELECTIVE ORTHOGONALIZATION AND
C SUBROUTINE SNPPLA WHICH POST PROCESSES THE OUTPUT OF SNWLA.
C HOWEVER SNLASO DOES CHECK FOR INCONSISTENCIES IN THE CALLING
C PARAMETERS AND DOES PREPROCESS ANY USER SUPPLIED EIGENPAIRS.
C SNLASO ALWAYS LOOKS FOR THE SMALLEST (LEFTMOST) EIGENVALUES.  IF
C THE LARGEST EIGENVALUES ARE DESIRED SNLASO IMPLICITLY USES THE
...
C     ABS(NVAL)  IS THE NUMBER OF EIGENVALUES DESIRED.
...
C     BE ZERO.  ABS(NVAL) MUST BE LESS THAN  MAXJ/2.
C
...
C     THAN OR EQUAL TO ABS(NVAL).
C
C   VAL   A TWO DIMENSIONAL REAL ARRAY OF ROW
...
C
C   VEC   A TWO DIMENSIONAL REAL ARRAY OF ROW
C     DIMENSION NMVEC AND COLUMN DIMENSION AT LEAST ABS(NVAL).  IF
...
C     OP.  SNLASO TERMINATES WHEN MAXOP IS EXCEEDED.  SEE
...
C
C   WORK   A REAL ARRAY OF DIMENSION AT LEAST AS
C     LARGE AS
C
...
C
C   IND   AN INTEGER ARRAY OF DIMENSION AT LEAST ABS(NVAL).
C
...
C     FOR RESTARTING THE ALGORITHM AND SNLASO CAN BE IMMEDIATELY
...
C
     ...
     *  I12, I13, M, NBAND, NOP, NV, IABS, MAX0
      ...
      REAL DELTA, EPS, TEMP, SDOT, SNRM2, ABS, TARR(1)
      EXTERNAL SNPPLA, SNWLA, SMVPC, SORTQR, SAXPY,
     *  SCOPY, SDOT, SNRM2, SSCAL, SSWAP, SLAEIG, SLABCM,
     *  SLABFC, SLAGER, SLARAN, SVSORT
C
C NOP   RETURNED FROM SNWLA AS THE NUMBER OF CALLS TO THE
C   SUBROUTINE OP.
C
C NV   SET EQUAL TO ABS(NVAL), THE NUMBER OF EIGENVALUES DESIRED,
C   AND PASSED TO SNWLA.
C
C SMALL   SET TO .TRUE. IF THE SMALLEST EIGENVALUES ARE DESIRED.
C
C RARITZ   RETURNED FROM SNWLA AND PASSED TO SNPPLA.  RARITZ IS .TRUE.
C   IF A FINAL RAYLEIGH-RITZ PROCEDURE IS NEEDED.
C
C DELTA   RETURNED FROM SNWLA AS THE EIGENVALUE OF THE MATRIX
C   WHICH IS CLOSEST TO THE DESIRED EIGENVALUES.
C
C SNPPLA   A SUBROUTINE FOR POST-PROCESSING THE EIGENVECTORS COMPUTED
C   BY SNWLA.
C
C SNWLA   A SUBROUTINE FOR IMPLEMENTING THE LANCZOS ALGORITHM
C   WITH SELECTIVE ORTHOGONALIZATION.
C
C SMVPC   A SUBROUTINE FOR COMPUTING THE RESIDUAL NORM AND
C   ORTHOGONALITY COEFFICIENT OF GIVEN RITZ VECTORS.
C
C SORTQR   A SUBROUTINE FOR ORTHONORMALIZING A BLOCK OF VECTORS
C   USING HOUSEHOLDER REFLECTIONS.
C
C SAXPY,SCOPY,SDOT,SNRM2,SSCAL,SSWAP   A SUBSET OF THE BASIC LINEAR
C   ALGEBRA SUBPROGRAMS USED FOR VECTOR MANIPULATION.
C
C SLARAN   A SUBROUTINE TO GENERATE RANDOM VECTORS
C
C SLAEIG, SLAGER, SLABCM, SLABFC   SUBROUTINES FOR BAND EIGENVALUE
C   CALCULATIONS.
C
C ------------------------------------------------------------------
C
...
C IF A USER SUPPLIED VECTOR IS ZERO OR IF SIGNIFICANT CANCELLATION
C OCCURS IN THE ORTHOGONALIZATION PROCESS THEN IERR IS SET TO  -1
C AND SNLASO TERMINATES.
C
      IF (NPERM.EQ.0) GO TO 110
C
C THIS NEGATES THE USER SUPPLIED EIGENVALUES WHEN THE LARGEST
C EIGENVALUES ARE DESIRED, SINCE SNWLA WILL IMPLICITLY USE THE
C NEGATIVE OF THE MATRIX.
C
      IF (SMALL) GO TO 20
      DO 10 I=1,NPERM
...
C
   20 CALL SVSORT(NPERM, VAL, VAL(1,2), 0, TARR, NMVEC, N, VEC)
C
...
C
      DO 60 I=1,NPERM
         VAL(I,2) = ABS(VAL(I,2))
         VAL(I,3) = SNRM2(N,VEC(1,I),1)
   60 CONTINUE
C
C THIS PERFORMS THE ORTHONORMALIZATION.
C
      M = N*NBLOCK + 1
      CALL SORTQR(NMVEC, N, NPERM, VEC, WORK(M))
      M = N*NBLOCK - NPERM
      DO 70 I = 1, NPERM
         M = M + NPERM + 1
         IF(ABS(WORK(M)) .GT. 0.9*VAL(I,3)) GO TO 70
         IERR = -1
         RETURN
C
   70 CONTINUE
...
C THE ARRAY WORK FOR LATER REFERENCE IN SNWLA.
C
      M = 2*N*NBLOCK + 1
      CALL SCOPY(NPERM, VAL(1,2), 1, WORK(M), 1)
C
C THIS SETS EPS TO AN APPROXIMATION OF THE RELATIVE MACHINE
C PRECISION
C
C ***THIS SHOULD BE REPLACED BY AN ASSIGNMENT STATEMENT
...
C
  110 EPS = 1.0 
      DO 120 I = 1,1000
         EPS = 0.5 *EPS
         TEMP = 1.0  + EPS
         IF(TEMP.EQ.1.0 ) GO TO 130
  120 CONTINUE
C
C ------------------------------------------------------------------
C
C THIS SECTION CALLS SNWLA WHICH IMPLEMENTS THE LANCZOS ALGORITHM
C WITH SELECTIVE ORTHOGONALIZATION.
C
  130 NBAND = NBLOCK + 1
      I1 = 1 + N*NBLOCK
      ...
      I13 = I12 + MAXJ
      CALL SNWLA(OP, IOVECT, N, NBAND, NV, NFIG, NPERM, VAL, NMVEC,
...
C ------------------------------------------------------------------
C
C THIS SECTION CALLS SNPPLA (THE POST PROCESSOR).
C
      ...
      I5 = I4 + N*NBLOCK
      I6 = I5 + 2*NPERM + 4
      CALL SNPPLA(OP, IOVECT, N, NPERM, NOP, NMVAL, VAL, NMVEC,
...
C ------------------------------------------------------------------
C
      SUBROUTINE SNWLA(OP, IOVECT, N, NBAND, NVAL, NFIG, NPERM, VAL,
...
C
     ...
     *   NLEFT, NSTART, NTHETA, NUMBER, MIN0, MTEMP
      LOGICAL ENOUGH, TEST
      REAL ALPMAX, ALPMIN, ANORM, BETMAX, BETMIN,
     *   EPSRT, PNORM, RNORM, TEMP,
     *   TMAX, TMIN, TOLA, TOLG, UTOL, ABS,
     *   AMAX1, AMIN1, SQRT, SDOT, SNRM2, TARR(1), ZERO(1)
      EXTERNAL SMVPC, SORTQR, SAXPY, SCOPY, SDOT,
     *   SNRM2, SSCAL, SSWAP, SLAEIG, SLAGER, SLARAN, SVSORT
C
...
C       SLAEIG
C
C ZERO  AN ARRAY OF LENGTH ONE CONTAINING ZERO, USED TO INSURE TYPE CONSISTENCY
C       IN CALLS TO SCOPY
C
      ZERO(1) = 0.0 
      RNORM = 0.0 
      IF (NPERM.NE.0) RNORM = AMAX1(-VAL(1),VAL(NPERM))
      PNORM = RNORM
      DELTA = 10.D30
      EPSRT = SQRT(EPS)
      NLEFT = NVAL - NPERM
      ...
      NUMBER = NPERM
      RARITZ = .FALSE.
      UTOL = AMAX1(FLOAT(N)*EPS,10.0 **-FLOAT(NFIG))
      J = MAXJ
C
C ------------------------------------------------------------------
C
...
C
   30 DO 50 I=1,NBLOCK
         TEMP = SNRM2(N,P1(1,I),1)
         IF (TEMP.EQ.0 ) CALL SLARAN(N, P1(1,I))
   50 CONTINUE
      ...
      DO 60 I=1,NPERM
         ...
         OTAU(I) = 0.0 
   60 CONTINUE
   70 CALL SCOPY(N*NBLOCK, ZERO, 0, P0, 1)
      CALL SCOPY(NBLOCK*NBLOCK, ZERO, 0, BET, 1)
      CALL SCOPY(J*NBAND, ZERO, 0, T, 1)
      MTEMP = NVAL + 1
      DO 75 I = 1, MTEMP
         CALL SCOPY(J, ZERO, 0, S(1,I), 1)
   75 CONTINUE
      NGOOD = 0
      TMIN = 1.0D30
      TMAX = -1.0D30
...
C
      ...
      DO 100 I=1,NUMBER
         ...
         TEST = .TRUE.
         TAU(I) = 0.0 
         IF (OTAU(I).NE.0.0 ) OTAU(I) = 1.0 
         DO 90 K=1,NBLOCK
            TEMP = -SDOT(N,VEC(1,I),1,P1(1,K),1)
            CALL SAXPY(N, TEMP, VEC(1,I), 1, P1(1,K), 1)
C
...
C
            IF (ABS(TEMP*BET(K,K)).GT.FLOAT(N)*EPSRT*
...
C
  110 IF(.NOT. TEST) GO TO 160
      CALL SORTQR(N, N, NBLOCK, P1, ALP)
      TEST = .FALSE.
      IF(J .EQ. NBLOCK) GO TO 160
      DO 130 I = 1,NBLOCK
         IF(ALP(I,I) .GT. 0.0 ) GO TO 130
         M = J - 2*NBLOCK + I
         L = NBLOCK + 1
         DO 120 K = I,NBLOCK
            BET(I,K) = -BET(I,K)
...
C
      DO 180 I=1,NBLOCK
         DO 170 K=I,NBLOCK
            CALL SAXPY(N, -BET(I,K), P0(1,K), 1, P2(1,I), 1)
  170    CONTINUE
  180 CONTINUE
C
...
C
      DO 200 I=1,NBLOCK
         DO 190 K=1,I
            II = I - K + 1
            ALP(II,K) = SDOT(N,P1(1,I),1,P2(1,K),1)
            CALL SAXPY(N, -ALP(II,K), P1(1,I), 1, P2(1,K), 1)
            IF (K.NE.I) CALL SAXPY(N, -ALP(II,K), P1(1,K),
     *        1, P2(1,I), 1)
  190   CONTINUE
  200 CONTINUE
C
...
C
      ...
      DO 215 I=1,NBLOCK
         DO 210 K=1,I
            TEMP = SDOT(N,P1(1,I),1,P2(1,K),1)
            CALL SAXPY(N, -TEMP, P1(1,I), 1, P2(1,K), 1)
            IF (K.NE.I) CALL SAXPY(N, -TEMP, P1(1,K),
     *        1, P2(1,I), 1)
            II = I - K + 1
...
C
  220 CALL SORTQR(N, N, NBLOCK, P2, BET)
C
...
C
  280 CALL SCOPY(NBLOCK*N, P1, 1, P0, 1)
      CALL SCOPY(NBLOCK*N, P2, 1, P1, 1)
      CALL SLAGER(J, NBAND, J-NBLOCK+1, T, TMIN, TMAX)
      ANORM = AMAX1(RNORM, TMAX, -TMIN)
      IF (NUMBER.EQ.0) GO TO 305
C
C THIS COMPUTES THE EXTREME EIGENVALUES OF ALP.
C
      CALL SCOPY(NBLOCK, ZERO, 0, P2, 1)
      CALL SLAEIG(NBLOCK, NBLOCK, 1, 1, ALP, TARR, NBLOCK,
     1   P2, BOUND, ATEMP, D, VTEMP, EPS, TMIN, TMAX)
      ALPMIN = TARR(1)
      CALL SCOPY(NBLOCK, ZERO, 0, P2, 1)
      CALL SLAEIG(NBLOCK, NBLOCK, NBLOCK, NBLOCK, ALP, TARR,
...
C
  305 DO 310 I = 1, NBLOCK
         DO 300 K = 1, I
            L = I - K + 1
            ALP(L,K) = SDOT(NBLOCK-I+1, BET(I,I), NBLOCK, BET(K,I),
     1        NBLOCK)
  300    CONTINUE
  310 CONTINUE
...
C
      CALL SCOPY(NBLOCK, ZERO, 0, P2, 1)
      CALL SLAEIG(NBLOCK, NBLOCK, 1, 1, ALP, TARR, NBLOCK,
     1  P2, BOUND, ATEMP, D, VTEMP, EPS, 0.0, ANORM*ANORM)
      BETMIN = SQRT(TARR(1))
C
C THIS UPDATES TAU AND OTAU.
C
      DO 320 I=1,NUMBER
         TEMP = (TAU(I)*AMAX1(ALPMAX-VAL(I),VAL(I)-ALPMIN)
...
C
  330 CALL SCOPY(NBLOCK, ZERO, 0, P2, 1)
      CALL SLAEIG(NBLOCK, NBLOCK, NBLOCK, NBLOCK, ALP, TARR,
     1  NBLOCK, P2, BOUND, ATEMP, D, VTEMP, EPS, 0.0,
     2  ANORM*ANORM)
      BETMAX = SQRT(TARR(1))
...
C
  ...
  400 NTHETA = MIN0(J/2,NLEFT+1)
      CALL SLAEIG(J, NBAND, 1, NTHETA, T, VAL(NUMBER+1),
     1  MAXJ, S, BOUND, ATEMP, D, VTEMP, EPS, TMIN, TMAX)
      CALL SMVPC(NBLOCK, BET, MAXJ, J, S, NTHETA, ATEMP, VTEMP, D)
C
...
C
      ...
      DELTA = AMIN1(DELTA,ANORM)
      PNORM = AMAX1(RNORM,AMAX1(-VAL(NUMBER+1),DELTA))
      TOLA = UTOL*PNORM
      NSTART = 0
      DO 460 I=1,NTHETA
         M = NUMBER + I
         IF (AMIN1(ATEMP(I)*ATEMP(I)/(DELTA-VAL(M)),ATEMP(I))
     *     .GT.TOLA) GO TO 450
         IND(I) = -1
         GO TO 460
C
...
C PAUSE.
C
      CALL SCOPY(NTHETA, VAL(NUMBER+1), 1, VAL(NPERM+1), 1)
      IF (NSTART.EQ.0) GO TO 580
      IF (NSTART.EQ.NTHETA) GO TO 530
      CALL SVSORT(NTHETA, VTEMP, ATEMP, 1, VAL(NPERM+1), MAXJ,
     *  J, S)
C
...
C
      TEMP = ATEMP(1)
      DO 535 I = 1, NSTART
         TEMP = AMIN1(TEMP, ATEMP(I))
  535 CONTINUE
      M = NGOOD + 1
      L = NGOOD + MIN0(NSTART,NBLOCK)
      DO 540 I=M,L
         CALL SSCAL(J, TEMP/ATEMP(I), S(1,I), 1)
  540 CONTINUE
      M = (NSTART-1)/NBLOCK
      IF (M.EQ.0) GO TO 570
      L = NGOOD + NBLOCK
      DO 560 I=1,M
         DO 550 K=1,NBLOCK
            ...
            I1 = NGOOD + K
            CALL SAXPY(J, TEMP/ATEMP(L), S(1,L), 1, S(1,I1), 1)
  550    CONTINUE
  560 CONTINUE
...
C
  600 NUMBER = NPERM + NGOOD
      IF (TEST .OR. ENOUGH) CALL SCOPY(N*NBLOCK, ZERO, 0, P1, 1)
  ...
  620 DO 670 I=NBLOCK,J,NBLOCK
         ...
         DO 660 K=1,NBLOCK
            ...
            DO 630 L=1,NSTART
               I1 = NGOOD + L
               CALL SAXPY(N, S(M,I1), P2(1,K), 1, P1(1,L), 1)
  630       CONTINUE
  640       IF (NGOOD.EQ.0) GO TO 660
            DO 650 L=1,NGOOD
               I1 = L + NPERM
               CALL SAXPY(N, S(M,L), P2(1,K), 1, VEC(1,I1), 1)
  650       CONTINUE
  660    CONTINUE
  670 CONTINUE
...
C
      M = NPERM + 1
      DO 680 I=M,NUMBER
         TEMP = 1.0 /SNRM2(N,VEC(1,I),1)
         CALL SSCAL(N, TEMP, VEC(1,I), 1)
         TAU(I) = 1.0 
         OTAU(I) = 1.0 
  680 CONTINUE
C
C  SHIFT S VECTORS TO ALIGN FOR LATER CALL TO SLAEIG
C
      CALL SCOPY(NTHETA, VAL(NPERM+1), 1, VTEMP, 1)
      CALL SVSORT(NTHETA, VTEMP, ATEMP, 0, TARR, MAXJ, J, S)
      GO TO 80
C
C ------------------------------------------------------------------
C
...
C
      CALL SORTQR(NMVEC, N, NPERM+NGOOD, VEC, S)
C
C THIS SORTS THE VALUES AND VECTORS.
C
      IF(NPERM .NE. 0) CALL SVSORT(NPERM+NGOOD, VAL, RES, 0, TEMP,
     *   NMVEC, N, VEC)
      NPERM = NPERM + NGOOD
      NLEFT = NLEFT - NGOOD
      RNORM = AMAX1(-VAL(1),VAL(NPERM))
C
