...
} ; //end of class.. Histogram for FixedLength MeasurementVectors




// This template specialization specializes the histogram to support
// variable length measurement vectors and the DenseFrequencyContainer.
// Below we support only float as the measurement vector type and the 
// frequency container as DenseFrequency container. Thanks to VS6 for not
// supporting partial template specialization. 
template <> 
class ITK_EXPORT Histogram < float, 
      MeasurementVectorTraits< Array< float > >::MeasurementVectorLength,
      DenseFrequencyContainer< float > >
  : public Sample < Array< float > >
{
public:

  /** Standard typedefs */
  typedef Histogram                       Self ;
  typedef float                           TMeasurement;
  ...
  /** frequency container typedef */
  typedef DenseFrequencyContainer< MeasurementType > FrequencyContainerType ;
   ...
   * The corresponding id is the offset of the index 
   * This method uses ImageBase::ComputeIndex() method */
  inline const IndexType & GetIndex(const InstanceIdentifier &id) const
    {
  ...
  /** Returns true if the given index is out of bound meaning one of index
   * is not between [0, last index] */
  inline bool IsIndexOutOfBounds(const IndexType &index) const
    {
  ...
  /** Get the minimum of the bin along dimension d corresponding to a
   * particular measurement. */
  inline const MeasurementType& GetBinMinFromValue(const unsigned int dimension, 
                                      const float value ) const  
    {
  ...
  /** Get the maximum of the bin along dimension d corresponding to a  
   * particular measurement. */
  inline const MeasurementType& GetBinMaxFromValue(const unsigned int dimension, 
                                      const float value ) const 
    {
  ...
  /** Get the minimums of the bin corresponding to a particular index */
  inline MeasurementVectorType& GetHistogramMinFromIndex(const IndexType &index) 
    {
    // Sanity check.. see if index is of the same length as MeasurementVectorSize;
    if( index.Size() != this->GetMeasurementVectorSize() )
      {
  ...
  /** Get the maximums of the bin corresponding to a particular index  */
  inline MeasurementVectorType& GetHistogramMaxFromIndex(const IndexType &index)  
    {
    // Sanity check.. see if index is of the same length as MeasurementVectorSize;
    if( index.Size() != this->GetMeasurementVectorSize() )
      {
  ...
  /** Get the frequency of an index */
  inline FrequencyType GetFrequency(const IndexType &index) const 
    {
    // Sanity check.. see if index is of the same length as MeasurementVectorSize;
    if( index.Size() != this->GetMeasurementVectorSize() )
      {
  ...
  /** Set all the bins in the histogram to a specified frequency */
  inline void SetFrequency(const FrequencyType value) 
    {
    Self::Iterator iter = this->Begin() ;
    Self::Iterator end = this->End() ;
  ...
  /** Set the frequency of an index. Returns false if the bin is
   * out of bounds. */
  inline bool SetFrequency(const IndexType &index, 
                    const FrequencyType value)
    {
    // Sanity check.. see if index is of the same length as MeasurementVectorSize;
    if( index.Size() != this->GetMeasurementVectorSize() )
  ...
  /** Set the frequency of a measurement. Returns false if the bin is
   * out of bounds. */
  inline bool SetFrequency(const MeasurementVectorType &measurement, 
                    const FrequencyType value)
    {
    // Sanity check.. see if index is of the same length as MeasurementVectorSize;
   ...
   * increased by the specified value. Returns false if the bin is out
   * of bounds. */
  inline bool IncreaseFrequency(const IndexType &index, 
                         const FrequencyType value) 
    {
    // Sanity check.. see if index is of the same length as MeasurementVectorSize;
    if( index.Size() != this->GetMeasurementVectorSize() )
   ...
   * increased by the specified value. Returns false if the
   * measurement is outside the bounds of the histogram. */
  inline bool IncreaseFrequency(const MeasurementVectorType &measurement, 
                         const FrequencyType value) 
    {
    // Sanity check.. see if index is of the same length as MeasurementVectorSize;
    const MeasurementVectorSizeType measurementVectorSize = 
   ...
   * centroid of the bin.
   */
  inline const MeasurementVectorType & GetMeasurementVector(const 
  ...
  /** Get the measurement of an index. This is the centroid of the bin. */
  inline const MeasurementVectorType & 
  ...
  /** Get the measurement a bin along a specified dimension.  This is
   * the midpoint of the bin along that dimension. */
  inline MeasurementType GetMeasurement(const unsigned long n,
                                  const unsigned int dimension) const 
    {
  ...
  /** Get the total frequency in the histogram*/
  inline FrequencyType GetTotalFrequency() const
    {
    return m_FrequencyContainer->GetTotalFrequency() ;
    }

  /** Get the frequency of a dimension's nth element. */
  inline FrequencyType GetFrequency(const unsigned long n,
                             const unsigned int dimension) const 
    {
...
}; // end of class Histogram (for Variable length Measurement vectors )
