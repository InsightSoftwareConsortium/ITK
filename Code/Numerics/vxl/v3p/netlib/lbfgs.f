...
C   ----------------------------------------------------------
C
C   These routines removed for insertion into TargetJr netlib
C
c awf       subroutine daxpy(n,da,dx,incx,dy,incy)
c awf c
c awf c     constant times a vector plus a vector.
c awf c     uses unrolled loops for increments equal to one.
c awf c     jack dongarra, linpack, 3/11/78.
c awf c
c awf       double precision dx(1),dy(1),da
c awf       integer i,incx,incy,ix,iy,m,mp1,n
c awf c
c awf       if(n.le.0)return
c awf       if (da .eq. 0.0d0) return
c awf       if(incx.eq.1.and.incy.eq.1)go to 20
c awf c
c awf c        code for unequal increments or equal increments
c awf c          not equal to 1
c awf c
c awf       ix = 1
c awf       iy = 1
c awf       if(incx.lt.0)ix = (-n+1)*incx + 1
c awf       if(incy.lt.0)iy = (-n+1)*incy + 1
c awf       do 10 i = 1,n
c awf         dy(iy) = dy(iy) + da*dx(ix)
c awf         ix = ix + incx
c awf         iy = iy + incy
c awf    10 continue
c awf       return
c awf c
c awf c        code for both increments equal to 1
c awf c
c awf c
c awf c        clean-up loop
c awf c
c awf    20 m = mod(n,4)
c awf       if( m .eq. 0 ) go to 40
c awf       do 30 i = 1,m
c awf         dy(i) = dy(i) + da*dx(i)
c awf    30 continue
c awf       if( n .lt. 4 ) return
c awf    40 mp1 = m + 1
c awf       do 50 i = mp1,n,4
c awf         dy(i) = dy(i) + da*dx(i)
c awf         dy(i + 1) = dy(i + 1) + da*dx(i + 1)
c awf         dy(i + 2) = dy(i + 2) + da*dx(i + 2)
c awf         dy(i + 3) = dy(i + 3) + da*dx(i + 3)
c awf    50 continue
c awf       return
c awf       end
c awf C
...
c awf C   ----------------------------------------------------------
c awf C
c awf       double precision function ddot(n,dx,incx,dy,incy)
c awf c
c awf c     forms the dot product of two vectors.
c awf c     uses unrolled loops for increments equal to one.
c awf c     jack dongarra, linpack, 3/11/78.
c awf c
c awf       double precision dx(1),dy(1),dtemp
c awf       integer i,incx,incy,ix,iy,m,mp1,n
c awf c
c awf       ddot = 0.0d0
c awf       dtemp = 0.0d0
...
c awf       if( m .eq. 0 ) go to 40
c awf       do 30 i = 1,m
c awf         dtemp = dtemp + dx(i)*dy(i)
c awf    30 continue
c awf       if( n .lt. 5 ) go to 60
c awf    40 mp1 = m + 1
c awf       do 50 i = mp1,n,5
c awf         dtemp = dtemp + dx(i)*dy(i) + dx(i + 1)*dy(i + 1) +
c awf      *   dx(i + 2)*dy(i + 2) + dx(i + 3)*dy(i + 3) + dx(i + 4)*dy(i + 4)
c awf    50 continue
c awf    60 ddot = dtemp
c awf       return
c awf       end
