...
::MedialNodeTripletCorrespondenceProcess()
{
  ...
  output =
    static_cast< typename MedialNodeTripletCorrespondenceProcess::OutputDataStructureType * >( this->MakeOutput(0).
                                                                                               GetPointer() );
...
::GenerateData()
{
  ...
  while ( NodeListIterator != m_InputDataStructure->m_NodeList->end() ) //iterate
                                                                        // through
                                                                        // NodeList
                                                                        // in
                                                                        // pair
    ...
    typename OutputDataStructureType::SecondaryNodeListType * TripletListPointer =
      new typename OutputDataStructureType::SecondaryNodeListType();

    for ( SecondaryListIterator1 = NodeListIterator->begin();
          SecondaryListIterator1 != NodeListIterator->end();
          SecondaryListIterator1++ )                                                                                                  //iterate
                                                                                                                                      // through
                                                                                                                                      // SecondaryNodeLists
                                                                                                                                      // in
                                                                                                                                      // pair
      ...
      while ( SecondaryListIterator2 != NodeListIterator->end() && counter3 < counter2 ) //concurrent
                                                                                         // iteration
                                                                                         // through
                                                                                         // SecondaryNodeList
        {                                                                                //with
        ...
        //If statement to make sure no two indices in the BASE triplet are the
        // same
        if ( NodeListIterator->GetIndex() != SecondaryListIterator1->GetIndex(0)
          ...
          typename OutputDataStructureType::CorrespondingListType * CorrespondingTripletListPointer =
            new typename OutputDataStructureType::CorrespondingListType();
          ...
          // finding every possible pair, and testing for triplet
          // correspondence.
          //iterate through CorrPairList of SecondaryListIterator1
          for ( CorrespondingListIterator1 = SecondaryListIterator1->begin();
                CorrespondingListIterator1 != SecondaryListIterator1->end();
                CorrespondingListIterator1++ )
            {
            //iterate through CorrPairList of SecondaryListIterator2
            for ( CorrespondingListIterator2 = SecondaryListIterator2->begin();
                  CorrespondingListIterator2 != SecondaryListIterator2->end();
                  CorrespondingListIterator2++ )
              {
              // Check NodeIndex[0] (base node correspondence) in the
              // corresponding pairs to make sure they are the
              ...
              if ( CorrespondingListIterator1->GetNodeIndex(0) == CorrespondingListIterator2->GetNodeIndex(0) )
                {
                // Check to see if the corresponding triplet has a duplicate
                // node.
                if ( CorrespondingListIterator1->GetNodeIndex(0) != CorrespondingListIterator1->GetNodeIndex(1)
                  ...
                  if ( BaseTripAdded == true )
                    ...
                    while ( CorrespondingTripletListIterator != CorrespondingTripletListPointer->end()
                            && DuplicateCorrespondingTriplet == false )
                      {
                      if ( ( CorrespondingListIterator1->GetNodeIndex(0) ==
                             CorrespondingTripletListIterator->GetNodeIndex(0)
                             || CorrespondingListIterator1->GetNodeIndex(1) ==
                             CorrespondingTripletListIterator->GetNodeIndex(0)
                             || CorrespondingListIterator2->GetNodeIndex(1) ==
                             CorrespondingTripletListIterator->GetNodeIndex(0) )
                           && ( CorrespondingListIterator1->GetNodeIndex(0) ==
                                CorrespondingTripletListIterator->GetNodeIndex(1)
                                || CorrespondingListIterator1->GetNodeIndex(1) ==
                                CorrespondingTripletListIterator->GetNodeIndex(1)
                                || CorrespondingListIterator2->GetNodeIndex(1) ==
                                CorrespondingTripletListIterator->GetNodeIndex(1) )
                           && ( CorrespondingListIterator1->GetNodeIndex(0) ==
                                CorrespondingTripletListIterator->GetNodeIndex(2)
                                || CorrespondingListIterator1->GetNodeIndex(1) ==
                                CorrespondingTripletListIterator->GetNodeIndex(2)
                                || CorrespondingListIterator2->GetNodeIndex(1) ==
                                CorrespondingTripletListIterator->GetNodeIndex(2) ) )
                  ...
                  if ( DuplicateCorrespondingTriplet == false )
                    ...
                    TemporaryDistanceA = m_DistanceMatrixA->get( SecondaryListIterator1->GetIndex(
                                                                   0), SecondaryListIterator2->GetIndex(0) );
                    TemporaryDistanceB = m_DistanceMatrixB->get( CorrespondingListIterator1->GetNodeIndex(
                                                                   1), CorrespondingListIterator2->GetNodeIndex(1) );

                    if ( TemporaryDistanceA <= ( TemporaryDistanceB + 0.1 ) && TemporaryDistanceA >=
                         ( TemporaryDistanceB - 0.1 ) )
                      {
                      // If this comes out true, we have found a corresponding
                      // triplet.
