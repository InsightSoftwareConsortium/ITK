...
//  TInputMesh::Pointer Normals(TInputMesh::New()); 
...
//  for (i = 0; i < npoints; i++) Normals->SetPoint (i, d);
...
::ComputeForce()
{
  ...
  const int npoints = Forces->GetNumberOfPoints()-2; 
...
::ComputeDt()
{
  ...
  const int ncells = Locations->GetNumberOfCells()-2;
