...
::MedialNodePairCorrespondenceProcess()
{
  ...
  output  =
    static_cast< typename MedialNodePairCorrespondenceProcess::DataStructureType * >( this->MakeOutput(0).GetPointer() );
...
::GenerateData()
{
  ...
  for ( int i = 0; i < m_Rows; ++i ) //iterate through rows in dist matrix A
    ...
    typename DataStructureType::SecondaryNodeListType * PairListPointer =
      new typename DataStructureType::SecondaryNodeListType();
    ...
    for ( int j = 0; j < m_Rows; ++j ) // Iterate through rows in dist matrix A.
      ...
      if ( j > i ) // Make sure we dont check for the same pair twice.
        ...
        typename DataStructureType::CorrespondingListType * CorrespondingPairListPointer =
          new typename DataStructureType::CorrespondingListType();

        //get distance between two nodes in question from DMatrixA
        TemporaryDistance = m_DistanceMatrixA->get(i, j);

        // Iterate through DMatrixB to find distances close enough to
        // TemporaryDistance.
        for ( int a = 0; a < m_Columns; ++a ) //iterate through rows in dist
                                              // matrix B
          {
          for ( int b = 0; b < m_Columns; ++b ) //iterate through columns in
                                                // dist matrix B
            {
            // If distance of pair in matrix B is close enough to pair in matrix
            // A.
            if ( ( m_DistanceMatrixB->get(a,
                                          b) > ( TemporaryDistance - 0.1 ) )
                 && ( m_DistanceMatrixB->get(a, b) < ( TemporaryDistance + 0.1 ) ) )
              {
              ...
              if ( m_CorrespondenceMatrix->get(i, a) >= 0.9 && m_CorrespondenceMatrix->get(j, b) >= 0.9 )
                ...
                // Here we run the signature metric on the pairs, as a final
                ...
                //Here we run the signature metric on the pairs, as a final test
                // of correspondence
                ...
                if ( result1 >= 0.95 && result2 >= 0.95 )
                  {
                  // If it isnt already there, add i-j to pair correspondence
                  // structure
                  // with pair a-b as a matching pair.  if it is, just add pair
                  // a-b
                  ...
                  if ( BasePairAdded == false )
                    ...
                    NodeType NewNode;
                    //index i corresponds to index a, so a is
                    // base_pair_node_index
                ...
                if ( result1 >= 0.95 && result2 >= 0.95 )
                  ...
                  if ( BasePairAdded == false )
                    ...
                    NodeType NewNode;
                    // Index i corresponds to index b, so b is
                    // base_pair_node_index
                    ...
                    BasePairAdded = true; //set pairAdded to true, as it has
                                          // just been added
                    }
                  else
                    {
