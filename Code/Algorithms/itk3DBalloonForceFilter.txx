/*=========================================================================
  ...
  Module:    itk3DBalloonForceFilter.txx
  Language:  C++
  Date:      $Date$
  Version:   $Revision$
...
//	  extends[2] += n[2];
    ...
    f[0] = t*80*f[0]*xs/*sqrt(f[0]*f[0]+f[1]*f[1])*/; 
    f[1] = t*80*f[1]*ys/*sqrt(f[0]*f[0]+f[1]*f[1])*/;
//	f[2] = t*80*f[2]*zs;
	f[2] = 0;
...
//  InputPointsContainerPointer			myOutput = m_Output->GetPoints();
//  myOutput->Reserve(m_NumNodes);
   
  typename TriCell::Pointer				insertCell = TriCell::New(); 

// use m_Normals as an intermediate container to add in the new nodes
...
/*
	while ( locations != myLocations->End() ) {
	  s = locations.Value();
	  if (((i-s[0])*(i-s[0]) + (j-s[1])*(j-s[1]) < 36) &&
		  ((i-s[0])*(i-s[0]) + (j-s[1])*(j-s[1]) > 0)) {
	    if ( dis > (i-s[0])*(i-s[0]) + (j-s[1])*(j-s[1]) ) {
		  dis = (i-s[0])*(i-s[0]) + (j-s[1])*(j-s[1]);
		  node = k;
		  d[0] = s[0];
		  d[1] = s[1];
		}
	...
	if ( node != -1 ) {
	  v1[0] = it.Get()*(i - d[0])/sqrt((i-d[0])*(i-d[0]) + (j-d[1])*(j-d[1]));
	  v1[1] = it.Get()*(j - d[1])/sqrt((i-d[0])*(i-d[0]) + (j-d[1])*(j-d[1]));
	  v1[2] = 0;

	  m_Forces->GetPoint (node, v2_pt); 

	  v2[0] += v1[0]; 
	  v2[1] += v1[1]; 
	  v2[2] += v1[2];

      m_Forces->SetPoint (node, v2);
	} 
*/
	...
	while ( k < slicediv ) {
	  s = locations.Value();
	  if ( ((i-s[0])*(i-s[0]) + (j-s[1])*(j-s[1]) < 25) &&
		  ((i-s[0])*(i-s[0]) + (j-s[1])*(j-s[1]) > 1)) {
		d = forces.Value();
		d[0] += 4*it.Get()*(i - s[0])/sqrt((i-s[0])*(i-s[0]) + (j-s[1])*(j-s[1]));
		d[1] += 4*it.Get()*(j - s[1])/sqrt((i-s[0])*(i-s[0]) + (j-s[1])*(j-s[1]));
		d[2] = 0;
		forces.Value() = d;
	  }
	  ++locations;
...
//	  d1[2] = d1[2]/l1;
	  ...
	  v2[1] = v2[1] + dis * d1[1];
	  v2[2] = v2[2]/* + dis * d1[2]*/;
//	  m_Forces->SetPoint(i, v2);
//	  m_Displacements->GetPoint (i, v2_pt);
//	  v2[0] = v2[0] + dis * d1[0];
//	  v2[1] = v2[1] + dis * d1[1];
//	  v2[2] = v2[2]/* + dis * d1[2]*/;
//	  m_Displacements->SetPoint(i, v2);
	}
