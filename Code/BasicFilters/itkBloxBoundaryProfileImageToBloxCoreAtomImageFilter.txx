...
{
  itkDebugMacro(
    << "BloxBoundaryProfileImageToBloxCoreAtomImageFilter::BloxBoundaryProfileImageToBloxCoreAtomImageFilter() called");
  m_DistanceMin = 8;
  m_DistanceMax = 12;
  m_Epsilon = 0;
  m_Polarity = 0;
...
::FindCoreAtomsAtBoundaryPoint(BloxProfileItemType *pBPOne)
{
  ...
  for ( int i = 0; i < 2; ++i )
    ...
    // spatialFunctionOrigin.GetVectorFromOrigin();

    VectorType spatialFunctionOriginVector;
    spatialFunctionOriginVector.SetVnlVector( spatialFunctionOrigin.GetVnlVector() );

    // Set the gradient of the conic shell to the current boundary point
    // gradient
      ...
      for ( sfi.GoToBegin(); !( sfi.IsAtEnd() ); ++sfi )
        ...
        for ( bpiterator = sfi.Get().begin(); bpiterator != sfi.Get().end(); ++bpiterator )
          ...
          if ( faceToFaceness > ( 1.0 - m_Epsilon ) )
            ...
            // Get the average pixel intensity in the pixels surrounding the
            // center of the core atom
            ...
            if ( faceness1 <= 0 && faceness2 <= 0 )
              ...
              std::cerr
              <<
              "BloxBoundaryProfileImageToBloxCoreAtomImageFilter::FindCoreAtomsAtBoundaryPoint: Invalid Homogeneous Condition"
              << std::endl;
              m_CreateCoreAtom = false;
              }
