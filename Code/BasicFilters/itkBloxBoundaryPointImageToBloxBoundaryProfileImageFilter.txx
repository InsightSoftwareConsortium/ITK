...
::BloxBoundaryPointImageToBloxBoundaryProfileImageFilter()
{
  ...
  itkDebugMacro(
    <<
    "itkBloxBoundaryPointImageToBloxBoundaryProfileImageFilter::itkBloxBoundaryPointImageToBloxBoundaryProfileImageFilter() called");
}
...
::GenerateData()
{
  ...
  // Pointers to the source image, the boundary point image, and the output
  // image
  ...
  for ( bpIt.GoToBegin(); !bpIt.IsAtEnd(); ++bpIt )
    ...
    for ( bpiterator = bpIt.Value().begin(); bpiterator != bpIt.Value().end(); ++bpiterator )
      ...
      // When constructing boundary profiles at a boundary point, we want to
      // sample
      ...
      // Set the origin of the spatial function to the current boundary point
      // location
      ...
      // Set the orientation of the ellipsoid to the current boundary point
      // gradient
      ...
      // Create a seed position for the spatial function iterator we'll use
      // shortly
      ...
      for (; !( sfi.IsAtEnd() ); ++sfi )
        ...
        // Gaussian Splat - Project Gaussian weighted pixel intensities along
        // major axis of ellipsoid (sampling region)
        if ( m_SplatMethod == 0 )
          {
          double a = 2;
          double b = .6; // for weight .5

          this->AddSplatToAccumulatorAndNormalizer(binNumber - 1,
                                                   double( a * vcl_exp( -.5 * ( vcl_pow( ( binJitter + 1 ) / b, 2 ) ) ) ),
                                                   sourcePixelValue);
          this->AddSplatToAccumulatorAndNormalizer(binNumber,
                                                   double( a * vcl_exp( -.5 * ( vcl_pow( ( binJitter  ) / b, 2 ) ) ) ),
                                                   sourcePixelValue);
          this->AddSplatToAccumulatorAndNormalizer(binNumber + 1,
                                                   double( a * vcl_exp( -.5 * ( vcl_pow( ( binJitter - 1 ) / b, 2 ) ) ) ),
                                                   sourcePixelValue);
          this->AddSplatToAccumulatorAndNormalizer(binNumber + 2,
                                                   double( a * vcl_exp( -.5 * ( vcl_pow( ( binJitter - 2 ) / b, 2 ) ) ) ),
                                                   sourcePixelValue);
          }

        // Triangle splat - Project Triangular weighted pixel intensities along
        // major axis of ellipsoid (sampling region)
      ...
      if ( this->FitProfile() == EXIT_SUCCESS )
        {
        // Create a new boundary profile if within constraints of imaging
        // modality
        if ( m_FinalParameters[0] >= 0 && m_FinalParameters[0] <= 255
          ...
          boundaryProfile->SetOptimalBoundaryLocation(
            spatialFunctionOriginVector.GetVnlVector(), orientationVNL.GetVnlVector() );
          ...
          // Figure out the data space coordinates of the optimal boundary
          // location
          ...
          if ( bufferedRegion.IsInside(boundaryProfilePosition) )
            {
            // Store the new boundary profile in the correct spot in output
            // image
