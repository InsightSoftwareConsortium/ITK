...
::GenerateData( void )
{
  ...
  // except for pixels with ErodeValue.  These pixels are initially
  // replaced with BackgroundValue and potentially replaced later with
  // ErodeValue as the Minkowski sums are performed.
  ...
  while ( !outIt.IsAtEnd() )
    {
    outIt.Set( static_cast<OutputPixelType>( foregroundValue ) );
    ++outIt;
    }
  ...
  obc.SetConstant( foregroundValue );
  ...
  while( !ouRegIndexIt.IsAtEnd() )
    ...
    if( inRegIt.Get() != foregroundValue )
      {
      ouRegIndexIt.Set( static_cast<OutputPixelType>( inRegIt.Get() ) );
      }
...
::PrintSelf( std::ostream& os, Indent indent) const
{
  ...
  os << indent << "Erode Value: " << static_cast<typename NumericTraits<InputPixelType>::PrintType>(this->GetForegroundValue()) << std::endl;
}
