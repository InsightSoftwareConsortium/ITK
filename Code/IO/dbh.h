...
#endif
/*
 * NOTE: The structure definitions are
 ...
 */
  /**
   * \struct header_key
   * \var sizeof_header      Must indicate the byte size of header file, Almost always 348 bytes, but may be larger
   *                         for sites that have extended the default implementation.
   *                         This feild is used by analyze to determine if the file is big endian or little endian.
   *                         If the size of the *.hdr file does not equal this value, then the structure needs
   *                         to be byte swapped.
   * \var data_type[10]  A convenience character string that has a 1 to 1 correspondence
   *                         with the short int datatype field of the image_dimension.
   *                         Acceptable values are:  
   *                         DataTypes[12][10]= { "UNKNOWN",
   *                         "BINARY","CHAR","SHORT", "INT","FLOAT","COMPLEX",
   *                         "DOUBLE","RGB", "ALL","USHORT","UINT" };
   * \var db_name[18]    A convenience character string that should be the same as the
   *                         the file name without th e*.hdr or *.img extension.
   * \var extents;        Should be 16384, the image file is created as contiguous with a
   *                         minium extent size.
   *                         This is the key for determining byte endedness of the file.
   * \var session_error; This feild is not used for anything other than internal use in Pre 1995 versions
   *                              of Analyze.  Setting this to 0 causes no problems.
   * \var regular;       This must be 'r' to indicate that all images and volumes are the same size.
   * \var hkey_un0;      Unused field for future expansion.
   */
...
{                              /* off + size*/
   ...
   *
   *  \var dim[]   array of image dimensions
   ...
   *
   *  \var vox_units[4] specifies the spatial units of measure for a voxel, valid values are "mm","cm" and "in"
   *                          // NOTE:  if no match is found, "mm" is assumed
   *  \var cal_units[4]  specifies the name of the calibration unit, valid values are "mm","cm" and "in"
   *                          // NOTE: if no match is found, "mm" is assumed
   *  \var unused Unused field for future expansion
   *  \var datatype A short int that has a 1 to 1 correspondence
   ...
   *                  ANALYZE_DT_RGB            =128,
   *                  ANALYZE_DT_ALL            =255,
   *                  ANALYZE_DT_UNSIGNED_SHORT =6, ##NOTE: This is not supported by the ANALYZE 7.5 file format!!
   *                  ANALYZE_DT_UNSIGNED_INT =12, ##NOTE: This is not supported by the ANALYZE 7.5 file format!!
   *                 };
   *                 NOTE: THIS MUST BE FILLED IN.  This is the field that most
   *                 applications use to determine the type.
   * \var bitpix   bits per pixel.  This field must agree with the datatype and
   *               data_type fields.    DataTypeSizes[11]={0,1,8,16,32,32,64,64,24,0,16};
   * \var dim_un0  Unused value for future expansion.
   * \var float pixdim[]  parallel array to dim giving voxel dimensions
   ...
   *       pixdim[7]
   *
   * \var vox_offset  byte offset in the .img file at which
   *                  voxels start. If value is negative
   *                  specifies that the absolute value
   *                  is applied for every image in the file.
   * \var roi_scale    The "ROI Scale Factor" was really added in by the developers of SPM.
   ...
   * \var funused1      Unused, for future expansion
   * \var funused2      Unused, for future expansion
   * \var cal_max      Specify the minimum of the range of the calibration values.
   * \var cal_min      Specify the minimum of the range of the calibration values.
   *                    The parameters 'cal_max' and 'cal_min' provided a mechanism for rescaling
   ...
   * \var compressed   Valid values are only 0 and 1.  A 0 value indicates that the file is
   *                    uncompressed.  A 1 value indicates that the *.img file has been compress with the standard
   *                    Unix 'compress' utility.
   * \var verified       This was only used internally by Pre 1995 versions of analyze.
   * \var glmax see glmin
   * \var glmin This is the global max/min for the entire volume image (or sets of volume
...
{                              /* off + size*/
   ...
   * \var descrip[80]  A place to put a short description of the data.
   * \var aux_file[24] A place to put the name of an auxillary file to
   ...
   * Analyze program to precondition data for fast display in the Movie module, where the images
   * were actually flipped vertically in order to accommodate the raster paint
   * order on older graphics devices.  The only cases you will encounter will
   ...
   *
   * \var originator[10] The person or group that generated this image
   * \var generated[10]  The date the image was generated.
   * \var scannum[10]    An instituion independent identifier for the image
   * \var patient_id[10] An instituion independant identifier for the subject of the scan
   * \var exp_date[10]   Experiment date - these were used for DSR experiments
   * \var exp_time[10]   Experiment time - these were used for DSR experiments
   * \var hist_un0[3]    Unused padding of the structure
   * \var views          Number of views in reconstruction - these were used for DSR experiments
   * \var vols_added;    Number of time points summed together - these were used for DSR experiments
   * \var start_field    Video field for first view used in reconstruction - these were used for DSR experiments
   * \var field_skip     Video field skip factor used in the reconstructed image - these were used for DSR experiments
   * \var omax           The omax/omin and smax/smin relate to rescaling of
 ...
 * \var hk The header_key structure
 * \var dime The image_dimension structure
 * \var hist The data_history structure
 */
struct dsr                     /*      dsr              */
{                              /* off + size*/
