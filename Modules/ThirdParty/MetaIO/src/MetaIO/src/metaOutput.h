...
public:
  ...
  bool
  AddListField(std::string name, std::string description, ListType list);
  ...
  void
  AddStreamFile(const char * name, const char * filename);

  /** Enable or Disable a stream */
  void
  EnableStream(const char * name);
  void
  DisableStream(const char * name);
...
private:
  ...
  std::string
  GenerateXML(const char * filename = nullptr);
  std::string
  GetUsername(void);
