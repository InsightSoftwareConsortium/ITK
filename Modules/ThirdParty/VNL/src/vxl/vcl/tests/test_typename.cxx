...
public:
  ...
  vcl_test_typename()
  {
    if (vcl_test_typename_func(v) < 1)
      v = 1;
  }
};

inline int
vcl_test_typename_func(float /*v*/)
{
  return 1;
}
inline int
vcl_test_typename_func(double /*v*/)
{
  return 1;
}
template <class T>
inline int
vcl_test_typename_func(const vcl_test_vec<T> & v)
{
  return v.size();
}

int
test_typename_main(int /*argc*/, char * /*argv*/[])
{
  return 0;
}
