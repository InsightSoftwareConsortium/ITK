...
template <class T>
VNL_EXPORT m
element_product(const m &, const m &);
template <class T>
VNL_EXPORT m
element_quotient(const m &, const m &);
template <class T>
VNL_EXPORT T
dot_product(const m &, const m &);
template <class T>
VNL_EXPORT T
inner_product(const m &, const m &);
template <class T>
VNL_EXPORT T
cos_angle(const m &, const m &);
template <class T>
VNL_EXPORT std::ostream &
operator<<(std::ostream &, const m &);
template <class T>
VNL_EXPORT std::istream &
operator>>(std::istream &, m &);
#undef v
#undef m
...
public:
  ...
  // Complexity $O(r.c)$
  vnl_matrix(unsigned r, unsigned c, const T & v0); // r rows, c cols, value v0.
  ...
  // Complexity $O(n)$
  vnl_matrix(unsigned r, unsigned c, unsigned n, const T values[]); // use automatic arrays.
  ...
  // Complexity $O(r.c)$
  vnl_matrix(const T * data_block, unsigned r, unsigned c); // fill row-wise.

  //: Copy construct a matrix
  // Complexity $O(r.c)$
  vnl_matrix(const vnl_matrix<T> &); // from another matrix.
  ...
  inline void
  put(unsigned r, unsigned c, const T &);
  ...
  vnl_matrix<T>
  operator+(const vnl_matrix<T> & rhs) const
  {
    vnl_matrix<T> result(rhs.rows(), rhs.cols());
#ifndef NDEBUG
    if (this->num_rows != rhs.num_rows || this->num_cols != rhs.num_cols)
      vnl_error_matrix_dimension(
        "vnl_matrix<T>::operator+", this->num_rows, this->num_cols, rhs.num_rows, rhs.num_cols);
#endif

    const unsigned int n = this->num_rows * this->num_cols;
    const T * a = this->data[0];
    const T * b = rhs.data[0];
    T * dst = result.data[0];
  ...
  vnl_vector<T> apply_rowwise(T (*f)(const vnl_vector<T> &)) const;

  //: Make a vector by applying a function across columns.
  vnl_vector<T> apply_columnwise(T (*f)(const vnl_vector<T> &)) const;
  ...
  {
    return data ? data[0] + num_rows * num_cols : nullptr;
  }
  ...
  bool
  operator_eq(const vnl_matrix<T> & rhs) const;
