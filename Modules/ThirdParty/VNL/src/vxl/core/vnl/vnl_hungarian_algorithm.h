...
public:
  ...
  //: This constructor (and the following cast operator) is provided for backward compatibility with the original
  //: function implementation
  vnl_hungarian_algorithm(const vnl_matrix<T> & cost)
  {
    SetCostMatrix(cost);
    StartAssignment();
  }
  ...
  void
  SetCostMatrix(const vnl_matrix<T> &);
