...
double
vnl_real_polynomial::devaluate(double x) const
{
...
double
vnl_real_polynomial::evaluate_integral(double x1, double x2) const
{
...
vnl_real_polynomial::print(std::ostream & os) const
{
  ...
  for (int i = 0; i <= d; ++i)
  {
    if (coeffs_[i] == 0.0)
      continue;
    os << ' ';
    if (coeffs_[i] > 0.0 && !first_coeff)
      os << '+';
    if (i == d)
      os << coeffs_[i]; // the 0-degree coeff should always be output if not zero
