...
template <class T>
vnl_vector<T>
vnl_symmetric_eigensystem<T>::get_eigenvector(int i) const
{
...
template <class T>
T
vnl_symmetric_eigensystem<T>::get_eigenvalue(int i) const
{
  return D(i, i);
}
...
template <class T>
vnl_vector<T>
vnl_symmetric_eigensystem<T>::solve(const vnl_vector<T> & b)
{
...
template <class T>
T
vnl_symmetric_eigensystem<T>::determinant() const
{
  const int n = D.size();
  T det(1);
  for (int i = 0; i < n; ++i)
...
#define VNL_SYMMETRIC_EIGENSYSTEM_INSTANTIATE(T)                                                              \
  ...
  template VNL_ALGO_EXPORT bool vnl_symmetric_eigensystem_compute(                                            \
    vnl_matrix<T> const &, vnl_matrix<T> &, vnl_vector<T> &)
