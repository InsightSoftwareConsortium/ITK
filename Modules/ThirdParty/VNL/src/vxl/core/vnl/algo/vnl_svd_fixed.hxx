...
template <class T, unsigned int R, unsigned int C>
std::ostream &
operator<<(std::ostream & s, const vnl_svd_fixed<T, R, C> & svd)
{
...
template <class T, unsigned int R, unsigned int C>
void
vnl_svd_fixed<T, R, C>::zero_out_absolute(double tol)
{
  last_tol_ = tol;
  rank_ = C;
...
template <class T, unsigned int R, unsigned int C>
void
vnl_svd_fixed<T, R, C>::zero_out_relative(double tol) // sqrt(machine epsilon)
{
...
template <class T, unsigned int R, unsigned int C>
typename vnl_svd_fixed<T, R, C>::singval_t
vnl_svd_fixed<T, R, C>::determinant_magnitude() const
{
  if (!warned_f() && R != C)
...
template <class T, unsigned int R, unsigned int C>
typename vnl_svd_fixed<T, R, C>::singval_t
vnl_svd_fixed<T, R, C>::norm() const
{
...
vnl_svd_fixed<T, R, C>::recompose(unsigned int rnk) const
{
  if (rnk > rank_)
    rnk = rank_;
...
vnl_svd_fixed<T, R, C>::pinverse(unsigned int rnk) const
{
  if (rnk > rank_)
    rnk = rank_;
...
template <class T, unsigned int R, unsigned int C>
vnl_matrix_fixed<T, R, C>
vnl_svd_fixed<T, R, C>::tinverse(unsigned int rnk) const
{
  if (rnk > rank_)
    rnk = rank_;
  vnl_diag_matrix_fixed<T, C> W_inverse(Winverse_);
...
template <class T, unsigned int R, unsigned int C>
vnl_matrix<T>
vnl_svd_fixed<T, R, C>::solve(const vnl_matrix<T> & B) const
{
...
vnl_svd_fixed<T, R, C>::solve(const vnl_vector_fixed<T, R> & y) const
{
...
void
vnl_svd_fixed<T, R, C>::solve(const T * y, T * x) const
{
...
template <class T, unsigned int R, unsigned int C>
void
vnl_svd_fixed<T, R, C>::solve_preinverted(const vnl_vector_fixed<T, R> & y, vnl_vector_fixed<T, C> * x_out) const
{
  vnl_vector_fixed<T, C> x; // solution matrix
  x = U_.conjugate_transpose() * y;
...
template <class T, unsigned int R, unsigned int C>
vnl_matrix<T>
vnl_svd_fixed<T, R, C>::nullspace(int required_nullspace_dimension) const
{
...
template <class T, unsigned int R, unsigned int C>
vnl_matrix<T>
vnl_svd_fixed<T, R, C>::left_nullspace(int /*required_nullspace_dimension*/) const
{
...
template <class T, unsigned int R, unsigned int C>
vnl_vector_fixed<T, C>
vnl_svd_fixed<T, R, C>::nullvector() const
{
...
vnl_svd_fixed<T, R, C>::left_nullvector() const
{
