...
vnl_convolve_cyclic_using_fft(const vnl_vector<T1> & v1, const vnl_vector<T2> & v2, U *)
{
  ...
  for (unsigned i = 0; i < n; ++i)
    w1[i] = v1[i];
  vnl_vector<C> w2(n, C(0));
  for (unsigned i = 0; i < n; ++i)
    w2[i] = v2[i];

  vnl_fft_1d<double> fft(n);
  fft.fwd_transform(w1);
  fft.fwd_transform(w2);
...
vnl_convolve_cyclic(const vnl_vector<T1> & v1, const vnl_vector<T2> & v2, U *, bool use_fft)
{
  ...
  if (n == 0)
    return vnl_vector<U>(0, U(0));
  if (n == 1)
    return vnl_vector<U>(1, U(v1[0] * v2[0]));
...
inline bool
has_only_primefactors_2_3_5(unsigned int n)
{
  if (n <= 1)
    return true;
  while (n % 2 == 0)
...
template <class T1, class T2, class U>
inline vnl_vector<U>
vnl_convolve_using_fft(const vnl_vector<T1> & v1, const vnl_vector<T2> & v2, U *, int n)
{
  if (n + 1 < int(v1.size() + v2.size()))
    n = v1.size() + v2.size() - 1;

  // Make sure n has only prime factors 2, 3 and 5; if necessary, increase n.
  while (!has_only_primefactors_2_3_5(n))
    ++n;

  // pad with zeros, so the cyclic convolution is a convolution:
  vnl_vector<U> w1(n, U(0));
  for (unsigned i = 0; i < v1.size(); ++i)
    w1[i] = U(v1[i]);
  vnl_vector<U> w2(n, U(0));
