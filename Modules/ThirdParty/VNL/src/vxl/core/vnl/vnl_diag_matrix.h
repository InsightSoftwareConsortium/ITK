...
operator*(const vnl_diag_matrix<T> &, const vnl_vector<T> &);
...
public:
  ...
  vnl_diag_matrix &
  operator=(const vnl_diag_matrix<T> &) = default;
  vnl_diag_matrix &
  operator=(vnl_diag_matrix<T> &&) = default;
  ~vnl_diag_matrix() = default;
  ...
  vnl_diag_matrix(unsigned nn, const T & value)
    : diagonal_(nn, value)
  {}

  //: Construct a diagonal matrix from a vnl_vector.
  //  The vector elements become the diagonal elements.
  vnl_diag_matrix(vnl_vector<T> that)
    : diagonal_(std::move(that))
  {}
  ...
  determinant() const;
  vnl_vector<T>
  solve(const vnl_vector<T> & b) const;
  void
  solve(const vnl_vector<T> & b, vnl_vector<T> * out) const;
  ...
  inline vnl_diag_matrix &
  set_diagonal(const vnl_vector<T> & v)
  {
    diagonal_ = v;
    return *this;
  }
...
operator<<(std::ostream &, const vnl_diag_matrix<T> &);
...
template <class T>
inline T
vnl_diag_matrix<T>::determinant() const
{
  T det = T(1);
...
template <class T>
inline vnl_diag_matrix<T>
operator*(const vnl_diag_matrix<T> & A, const vnl_diag_matrix<T> & B)
{
...
template <class T>
inline vnl_matrix<T>
operator*(const vnl_matrix<T> & A, const vnl_diag_matrix<T> & D)
{
...
template <class T>
inline vnl_matrix<T>
operator*(const vnl_diag_matrix<T> & D, const vnl_matrix<T> & A)
{
...
template <class T>
inline vnl_vector<T>
operator*(const vnl_diag_matrix<T> & D, const vnl_vector<T> & A)
{
...
template <class T>
inline vnl_vector<T>
operator*(const vnl_vector<T> & A, const vnl_diag_matrix<T> & D)
{
  assert(A.size() == D.size());
  return element_product(D.diagonal(), A);
}
