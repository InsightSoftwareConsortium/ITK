...
local_ldexp(double x, int n)
{
  ...
  if (k <= -54)
  {
    if (n > 50000)                           /* in case integer overflow in n+k */
      ...
      return tiny * local_copysign(tiny, x); /*underflow*/
  }
...
static std::uint16_t _rand48_seed[3] = { RAND48_SEED_0, RAND48_SEED_1, RAND48_SEED_2 };
static std::uint16_t _rand48_mult[3] = { RAND48_MULT_0, RAND48_MULT_1, RAND48_MULT_2 };
...
_dorand48(std::uint16_t xseed[3])
{
  ...
  std::uint32_t accu = std::uint32_t{ _rand48_mult[0] } * std::uint32_t{ xseed[0] } + std::uint32_t{ _rand48_add };
...
{
  _dorand48(xseed);
  return local_ldexp(static_cast<double>(xseed[0]), -48) + local_ldexp(static_cast<double>(xseed[1]), -32) +
