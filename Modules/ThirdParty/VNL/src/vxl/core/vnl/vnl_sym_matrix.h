...
public:
  ...
  inline vnl_sym_matrix(const T * data, unsigned nn);
  ...
  inline explicit vnl_sym_matrix(const vnl_matrix<T> & that);

  //: Copy constructor
  inline vnl_sym_matrix(const vnl_sym_matrix<T> & that);

  ~vnl_sym_matrix();

  vnl_sym_matrix<T> &
  operator=(const vnl_sym_matrix<T> & that);

  // Operations----------------------------------------------------------------

  //: In-place arithmetic operations
  inline vnl_sym_matrix<T> &
  operator*=(T v)
  {
    vnl_c_vector<T>::scale(data_, data_, size(), v);
    return *this;
  }
  //: In-place arithmetic operations
  inline vnl_sym_matrix<T> &
  operator/=(T v)
  {
    vnl_c_vector<T>::scale(data_, data_, size(), ((T)1) / v);
    return *this;
  }
  ...
  void
  set_half_row(const vnl_vector<T> & half_row, unsigned i);
  ...
  vnl_sym_matrix<T> &
  update(const vnl_sym_matrix<T> & m, unsigned diag_start = 0);

  //: Swap contents of m with THIS
  void
  swap(vnl_sym_matrix & m) noexcept;
...
operator<<(std::ostream &, const vnl_sym_matrix<T> &);


template <class T>
inline vnl_sym_matrix<T>::vnl_sym_matrix(const T * data, unsigned nn)
  : data_(vnl_c_vector<T>::allocate_T(nn * (nn + 1) / 2))
  , index_(vnl_c_vector<T>::allocate_Tptr(nn))
  , nn_(nn)
{
...
template <class T>
inline vnl_sym_matrix<T>::vnl_sym_matrix(const vnl_matrix<T> & that)
...
template <class T>
inline vnl_sym_matrix<T>::vnl_sym_matrix(const vnl_sym_matrix<T> & that)
  : data_(nullptr)
  , index_(nullptr)
{
  set_size(that.rows());
...
template <class T>
inline vnl_matrix<T>
vnl_sym_matrix<T>::as_matrix() const
{
...
template <class T>
inline void
vnl_sym_matrix<T>::set_size(int n)
{
  if (n == (int)nn_)
    return;
...
template <class T>
VNL_EXPORT bool
operator==(const vnl_sym_matrix<T> & a, const vnl_sym_matrix<T> & b);

template <class T>
VNL_EXPORT bool
operator==(const vnl_sym_matrix<T> & a, const vnl_matrix<T> & b);

template <class T>
VNL_EXPORT bool
operator==(const vnl_matrix<T> & a, const vnl_sym_matrix<T> & b);

//: Swap the contents of a and b.
// \relatesalso vnl_sym_matrix
template <class T>
inline void
swap(vnl_sym_matrix<T> & a, vnl_sym_matrix<T> & b) noexcept
{
  a.swap(b);
}
