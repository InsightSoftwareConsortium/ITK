...
template <class T>
VNL_EXPORT T
inner_product(const v &, const v &);
template <class T>
VNL_EXPORT T
bracket(const v &, const m &, const v &);
template <class T>
VNL_EXPORT T
cos_angle(const v &, const v &);
template <class T>
VNL_EXPORT double
angle(const v &, const v &);
template <class T>
VNL_EXPORT m
outer_product(const v &, const v &);
template <class T>
VNL_EXPORT v
operator+(T, const v &);
template <class T>
...
template <class T>
VNL_EXPORT v
operator*(const v &, const m &);
template <class T>
VNL_EXPORT v
element_product(const v &, const v &);
template <class T>
VNL_EXPORT v
element_quotient(const v &, const v &);
template <class T>
VNL_EXPORT T
vnl_vector_ssd(const v &, const v &);
template <class T>
VNL_EXPORT void
swap(v &, v &) noexcept;
#undef v
...
public:
  ...
  operator+(T v) const
  {
    ...
    std::transform(this->begin(), this->end(), result.begin(), [v](T d) -> T { return d + v; });
    return result;
  }
  vnl_vector<T>
  operator-(T v) const
...
#ifndef NDEBUG
    if (this->size() != v.size())
      vnl_error_vector_dimension("vnl_vector<>::operator+()", this->size(), v.size());
#endif
...
#ifndef NDEBUG
    if (this->size() != M.rows())
      vnl_error_vector_dimension("vnl_vector<>::operator*(M)", this->size(), M.rows());
#endif
    vnl_sse<T>::vector_x_matrix(this->begin(), M.begin(), result.begin(), M.rows(), M.cols());
    return result;
  }
  ...
  bool
  is_equal(const vnl_vector<T> & rhs, double tol) const;
  ...
  //: Equality test
  bool
  operator==(const vnl_vector<T> & that) const
  {
...
protected:
   ...
   */
  void
  protected_set_data(T * indata, size_t nelmts, bool manage_own_memory)
  {
    this->data = indata;
...
template <class T>
inline vnl_vector<T>
operator*(const vnl_matrix<T> & M, const vnl_vector<T> & v)
{
...
template <class T>
inline vnl_vector<T>
operator+(T s, const vnl_vector<T> & v)
{
  return v.operator+(s);
}
...
template <class T>
inline T
vnl_vector_ssd(const vnl_vector<T> & v1, const vnl_vector<T> & v2)
{
#ifndef NDEBUG
  if (v1.size() != v2.size())
...
operator<<(std::ostream &, const vnl_vector<T> &);
