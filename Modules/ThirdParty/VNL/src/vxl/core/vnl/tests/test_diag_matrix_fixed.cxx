...
test_diag_matrix_fixed()
{
  ...
  const vnl_diag_matrix_fixed<double, N> m1c = m1; // must be const in order to use m(i,j)
  std::cout << "m1 = " << m1 << " =\n";
  for (unsigned i = 0; i < N; i++)
  {
    std::cout << '\t';
  ...
  const vnl_diag_matrix_fixed<double, N> m2c = m2;
  std::cout << "m2 = " << m2 << " =\n";
  for (unsigned i = 0; i < N; i++)
  {
    std::cout << '\t';
  ...
  TEST("matrix sum",
       s1(0, 0) == -0.25 && s1(0, 1) == -6.0 && s1(0, 2) == -3.0 && s1(1, 0) == 6.0 && s1(1, 1) == 0.0 &&
         s1(1, 2) == -2.0 && s1(2, 0) == 3.0 && s1(2, 1) == 2.0 && s1(2, 2) == 0.25,
       true);
  ...
  std::cout << "matrix-m1 =\n" << d2;
  TEST("matrix difference",
       d2(0, 0) == 0.25 && d2(0, 1) == -6.0 && d2(0, 2) == -3.0 && d2(1, 0) == 6.0 && d2(1, 1) == -0.0 &&
         d2(1, 2) == -2.0 && d2(2, 0) == 3.0 && d2(2, 1) == 2.0 && d2(2, 2) == -0.25,
       true);
  vnl_matrix_fixed<double, N, N> p1 = m1 * mat;
  std::cout << "m1*matrix =\n" << p1;
  TEST("matrix product",
       p1(0, 0) == 0.0 && p1(0, 1) == 1.5 && p1(0, 2) == 0.75 && p1(1, 0) == 0.0 && p1(1, 1) == 0.0 &&
         p1(1, 2) == 0.0 && p1(2, 0) == 0.75 && p1(2, 1) == 0.5 && p1(2, 2) == 0.0,
       true);
  vnl_matrix_fixed<double, N, N> p2 = mat * m1;
  std::cout << "matrix*m1 =\n" << p2;
  TEST("matrix product",
       p2(0, 0) == 0.0 && p2(0, 1) == 0.0 && p2(0, 2) == -0.75 && p2(1, 0) == -1.5 && p2(1, 1) == 0.0 &&
         p2(1, 2) == -0.5 && p2(2, 0) == -0.75 && p2(2, 1) == 0.0 && p2(2, 2) == 0.0,
       true);
