...
void test_polynomial_double()
{
  ...
  f3 = f2/f1;
  vcl_cout << "f2/f1 =" << f3 << vcl_endl; // should be X+1
  TEST("f2(x)/f1(x)", f3.evaluate(2.5), 3.5);
  TEST("f2/f1", f3.degree()==1 && f3[0]==1 && f3[1]==1, true);

  f3 = f2%f1;
  vcl_cout << "f2%f1 =" << f3 << vcl_endl; // should be 5
  ...
  vcl_vector<double> v; v.push_back(1.0); v.push_back(2.0); v.push_back(3.0);
  ...
  vcl_cout << "(f1*f2)/f2 =" << (f1*f2)/f2 << vcl_endl;
  TEST("(f1*f2)/f2 == f1", (f1*f2)/f2, f1);
  vcl_cout << "(f1*f2)/f1 =" << (f1*f2)/f1 << vcl_endl;
...
void test_polynomial_rational()
{
  ...
  vcl_vector<vnl_rational> v; v.push_back(1); v.push_back(2); v.push_back(3);
