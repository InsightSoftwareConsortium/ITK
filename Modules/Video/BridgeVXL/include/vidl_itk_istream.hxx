...
vidl_itk_istream<TVideoStream>::is_valid() const
{
  ...
  return (m_VideoStream == nullptr || m_AdvanceCalled);
}
...
vidl_itk_istream<TVideoStream>::is_seekable() const
{
  ...
  // true otherwise
  return (m_VideoStream->GetLargestPossibleTemporalRegion().GetFrameDuration() != ITK_INFINITE_FRAME_DURATION);
}
...
vidl_itk_istream<TVideoStream>::format() const
{
  ...
  // char / unsigned char
  else if (typeid(PixelValueType) == typeid(char) || typeid(PixelValueType) == typeid(unsigned char))
  {
    if (channels == 1)
    {
  ...
  else if (typeid(PixelValueType) == typeid(short) || typeid(PixelValueType) == typeid(unsigned short))
  {
    if (channels == 1)
    {
...
vidl_itk_istream<TVideoStream>::current_frame()
{
  ...
  vidl_frame_sptr output_frame = new vidl_shared_frame(
    static_cast<void *>(frame->GetBufferPointer()), this->width(), this->height(), this->format());
...
vidl_itk_istream<TVideoStream>::seek_frame(unsigned int frameNumber)
{
  ...
  if (!this->is_open() || !this->is_seekable() || frameNumber >= static_cast<unsigned int>(this->num_frames()))
  {
    return false;
  }
