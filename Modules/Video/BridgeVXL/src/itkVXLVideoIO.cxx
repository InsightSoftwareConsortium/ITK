...
VXLVideoIO::Read(void * buffer)
{
  ...
  if (!this->PixelFormatSupported(this->m_PixelFormat))
  {
    ...
    if (this->m_NumberOfComponents == 4)
    {
      ...
      this->m_VIDLFrame = vidl_convert_frame(this->m_VIDLFrame, vidl_pixel_format_from_string(ss.str()));
    }
    ...
    else if (this->m_NumberOfComponents == 1)
    {
      std::stringstream ss;
      ss << "MONO " << pixelSize * 8;
      this->m_VIDLFrame = vidl_convert_frame(this->m_VIDLFrame, vidl_pixel_format_from_string(ss.str().c_str()));
    }
    else
    {
...
VXLVideoIO::Write(const void * buffer)
{
  ...
  this->m_VIDLFrame = new vidl_shared_frame(
    const_cast<void *>(buffer), this->m_Dimensions[0], this->m_Dimensions[1], this->m_PixelFormat);
