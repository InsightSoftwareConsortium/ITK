                         ...
                         double inFpS )
{
  ...
  if (opencvIO->GetFpS() < inFpS - epsilon || opencvIO->GetFpS() > inFpS + epsilon)
    {
    infoSet = false;
    paramMessage << "FpS mismatch: (expected) " << inFpS << " != (got) " << opencvIO->GetFpS()
      << std::endl;
    }
  ...
  if (opencvIO->GetFpS() != fps || opencvIO->GetDimensions(0) != width ||
