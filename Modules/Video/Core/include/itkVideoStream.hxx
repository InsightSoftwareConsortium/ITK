                                                              ...
                                                              typename TFrameType::RegionType region)
{
  ...
  const SizeValueType bufDur = m_BufferedTemporalRegion.GetFrameDuration();
...
VideoStream<TFrameType>::GetFrameLargestPossibleSpatialRegion(SizeValueType frameNumber) const
{
...
template <typename TFrameType>
const typename TFrameType::RegionType &
VideoStream<TFrameType>::GetFrameRequestedSpatialRegion(SizeValueType frameNumber) const
{
  // It seems that std::map's [] operator isn't const correct, so we need to
  // access this member from an non-const version of ourselves
...
template <typename TFrameType>
const typename TFrameType::RegionType &
VideoStream<TFrameType>::GetFrameBufferedSpatialRegion(SizeValueType frameNumber) const
{
  // It seems that std::map's [] operator isn't const correct, so we need to
  // access this member from an non-const version of ourselves
...
template <typename TFrameType>
void
VideoStream<TFrameType>::SetFrameSpacing(SizeValueType frameNumber, typename TFrameType::SpacingType spacing)
{
...
VideoStream<TFrameType>::GetFrameSpacing(SizeValueType frameNumber) const
{
  // It seems that std::map's [] operator isn't const correct, so we need to
  // access this member from an non-const version of ourselves
...
template <typename TFrameType>
void
VideoStream<TFrameType>::SetFrameOrigin(SizeValueType frameNumber, typename TFrameType::PointType origin)
{
...
VideoStream<TFrameType>::GetFrameOrigin(SizeValueType frameNumber) const
{
  // It seems that std::map's [] operator isn't const correct, so we need to
  // access this member from an non-const version of ourselves
...
VideoStream<TFrameType>::GetFrameDirection(SizeValueType frameNumber) const
{
  // It seems that std::map's [] operator isn't const correct, so we need to
  // access this member from an non-const version of ourselves
...
VideoStream<TFrameType>::InitializeEmptyFrames()
{
  ...
  const SizeValueType numFrames = m_RequestedTemporalRegion.GetFrameDuration();
  if (numFrames == 0)
  {
    return;
  }
  ...
  const SizeValueType startFrame = m_RequestedTemporalRegion.GetFrameStart();
  for (SizeValueType i = startFrame; i < startFrame + numFrames; ++i)
  {
    ...
    if (m_LargestPossibleSpatialRegionCache.find(i) != m_LargestPossibleSpatialRegionCache.end())
    {
      this->GetFrame(i)->SetLargestPossibleRegion(m_LargestPossibleSpatialRegionCache[i]);
    }
    if (m_RequestedSpatialRegionCache.find(i) != m_RequestedSpatialRegionCache.end())
    {
      this->GetFrame(i)->SetRequestedRegion(m_RequestedSpatialRegionCache[i]);
    }
    if (m_BufferedSpatialRegionCache.find(i) != m_BufferedSpatialRegionCache.end())
    {
...
VideoStream<TFrameType>::Graft(const DataObject * data)
{
  ...
  if (data)
  {
    ...
    this->SetLargestPossibleSpatialRegionCache(videoData->GetLargestPossibleSpatialRegionCache());
    this->SetRequestedSpatialRegionCache(videoData->GetRequestedSpatialRegionCache());
    this->SetBufferedSpatialRegionCache(videoData->GetBufferedSpatialRegionCache());
...
VideoStream<TFrameType>::SetAllLargestPossibleSpatialRegions(typename TFrameType::RegionType region)
{
  ...
  {
    const SizeValueType bufEnd = m_BufferedTemporalRegion.GetFrameStart() + m_BufferedTemporalRegion.GetFrameDuration();
    const SizeValueType reqEnd =
      m_RequestedTemporalRegion.GetFrameStart() + m_RequestedTemporalRegion.GetFrameDuration();
