...
void
FibHeapNode::operator=(FibHeapNode & RHS)
{
  FHN_Assign(RHS);
...
FibHeap::Union(FibHeap * OtherHeap)
{
  ...
  if (OtherHeap == NULL || OtherHeap->MinRoot == NULL)
    return;
  ...
  OtherHeap->NumNodes = OtherHeap->NumTrees = OtherHeap->NumMarkedNodes = 0;
...
int
FibHeap::DecreaseKey(FibHeapNode * theNode, FibHeapNode & NewKey)
{
...
void
FibHeap::_Link(FibHeapNode * y, FibHeapNode * x)
{
...
void
FibHeap::_AddToRootList(FibHeapNode * x)
{
  if (x->Mark)
    NumMarkedNodes--;
  x->Mark = 0;
...
void
FibHeap::_Cut(FibHeapNode * x, FibHeapNode * y)
{
  if (y->Child == x)
    y->Child = x->Right;
  if (y->Child == x)
...
void
FibHeap::_CascadingCut(FibHeapNode * y)
{
