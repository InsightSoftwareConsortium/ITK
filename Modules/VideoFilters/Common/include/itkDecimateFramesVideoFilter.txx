...
void
DecimateFramesVideoFilter<TVideoStream>::
SetPreservedFrameSpacing(unsigned long numFrames)
{
                     ...
                     int threadId)
{
  ...
  unsigned long outFrameNum = outReqTempRegion.GetFrameStart();

  typename InputVideoStreamType::TemporalRegionType inReqTempRegion =
    input->GetRequestedTemporalRegion();
  unsigned long inFrameNum = inReqTempRegion.GetFrameStart();
