 ...
 *
 * BoneMorphometryImageFilter computes bone morphometry features such as the percent bone volume [BVTV], the trabecular
 * thickness [TbTh], the trabecular separation [TbSp], the trabecular number [TbN], or the Bone Surface to Bone Volume
 * ration [BSBV]. To do so, the filter needs a 3D input scan and a threshold. All the voxel with an intensity higher
 * than the threshold will be considered as part of the bone. A mask can also be specified in order to have more precise
 * results (the morphometry will be computed only for the mask's voxels with a value of 1)
 *
...
class ITK_TEMPLATE_EXPORT BoneMorphometryImageFilter : public ImageToImageFilter<TInputImage, TInputImage>
{
public:
