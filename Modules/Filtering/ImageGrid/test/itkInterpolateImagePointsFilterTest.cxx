...
test2DInterpolateImagePointsFilter()
{
  ...
  // not change results.
  static constexpr double origin[]{ 5.5, 1.0 };
  static constexpr double spacing[]{ 5.1, 0.5 };
  image->SetOrigin(origin);
  image->SetSpacing(spacing);

  // Initialize the sample data
  static constexpr int    NPOINTS2{ 4 };             // number of points
  static constexpr double DEFAULTPIXELVALUE{ 1.23 }; // Arbitrary value to test setting

  static constexpr double xcoord[NPOINTS2]{ 0.1, 3.4, 4.0, 2.0 };
  static constexpr double ycoord[NPOINTS2]{ 0.2, 5.8, 6.0, 7.0 };
  static constexpr double truth[NPOINTS2]{ 151.650316034, 22.411473093, 36.2, DEFAULTPIXELVALUE };
  ...
  static constexpr CoordImage2DSizeType size{ 2, 2 };
  ...
  static constexpr InterpolatorType2D::PixelType defaultPixelValue{ DEFAULTPIXELVALUE };
...
test3DInterpolateImagePointsFilter()
{
  ...
  // Get results and compare for accuracy
  const ImageTypePtr3D outputImage = resamp->GetOutput();
...
{
  static constexpr ImageType2DSizeType size{ 7, 7 };
...
set3DData()
{
  ...
  static constexpr ImageType3D::PointValueType origin[]{ 1.0f, 4.0f, 2.0f };
