...
transformImage(const char * inputImageFileName, const char * outputImageFileName)
{
  ...
  using ForwardFilterType = itk::PocketFFTForwardFFTImageFilter<RealImageType, ComplexImageType>;
  auto forwardFilter = ForwardFilterType::New();
  forwardFilter->SetInput(reader->GetOutput());

  using ComplexFilterType = itk::PocketFFTComplexToComplexFFTImageFilter<ComplexImageType>;
  auto inverseComplexFilter = ComplexFilterType::New();

  ITK_EXERCISE_BASIC_OBJECT_METHODS(
    inverseComplexFilter, PocketFFTComplexToComplexFFTImageFilter, ComplexToComplexFFTImageFilter);
  ...
  using InverseFilterType = itk::PocketFFTInverseFFTImageFilter<ComplexImageType, RealImageType>;
...
int
itkPocketFFTComplexToComplexFFTImageFilterTest(int argc, char * argv[])
{
  if (argc < 4)
  {
