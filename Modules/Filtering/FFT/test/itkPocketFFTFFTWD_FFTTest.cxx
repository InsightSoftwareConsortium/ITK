...
// Compare FFT using PocketFFT and FFTW libraries. The test is performed for 2 3d
...
// and double.
int
itkPocketFFTFFTWD_FFTTest(int, char *[])
{
...
#  endif
  ...
  if ((test_fft_rtc<double, 1, itk::PocketFFTForwardFFTImageFilter<ImageD1>, itk::FFTWForwardFFTImageFilter<ImageD1>>(
        SizeOfDimensions1)) != 0)
    rval++;
  std::cerr << "PocketFFT-FFTWD:double,2 (4,4,4)" << std::endl;
  if ((test_fft_rtc<double, 2, itk::PocketFFTForwardFFTImageFilter<ImageD2>, itk::FFTWForwardFFTImageFilter<ImageD2>>(
        SizeOfDimensions1)) != 0)
    rval++;
  std::cerr << "PocketFFT-FFTWD:double,3 (4,4,4)" << std::endl;
  if ((test_fft_rtc<double, 3, itk::PocketFFTForwardFFTImageFilter<ImageD3>, itk::FFTWForwardFFTImageFilter<ImageD3>>(
        SizeOfDimensions1)) != 0)
    rval++;
