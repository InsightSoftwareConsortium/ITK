...
/** Compile-time predicates selecting which precisions a templated FFT
 * filter is registered for. Both default to enabled; an FFT backend
 * specializes the relevant predicate to std::false_type when its
 * matching precision is unavailable (e.g. FFTW without ITK_USE_FFTWD). */
