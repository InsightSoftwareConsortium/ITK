...
SLICImageFilter<TInputImage, TOutputImage, TDistancePixel>::BeforeThreadedGenerateData()
{
  ...
  {
    numberOfThreads =
      vnl_math_min(this->GetNumberOfThreads(), ProcessObject::MultiThreaderType::GetGlobalMaximumNumberOfThreads());
  }
...
template <typename TInputImage, typename TOutputImage, typename TDistancePixel>
void
SLICImageFilter<TInputImage, TOutputImage, TDistancePixel>::ThreadedPerturbClusters(const OutputImageRegionType &,
                                                                                    ThreadIdType id)
{
...
template <typename TInputImage, typename TOutputImage, typename TDistancePixel>
void
SLICImageFilter<TInputImage, TOutputImage, TDistancePixel>::ThreadedConnectivity(const OutputImageRegionType &,
                                                                                 ThreadIdType threadId)
{
  ...
  const size_t minSuperSize =
    std::accumulate(m_SuperGridSize.Begin(), m_SuperGridSize.End(), size_t(1), std::multiplies<size_t>()) / 4;
...
template <typename TInputImage, typename TOutputImage, typename TDistancePixel>
void
SLICImageFilter<TInputImage, TOutputImage, TDistancePixel>::ThreadedGenerateData(
  const OutputImageRegionType & outputRegionForThread,
  ThreadIdType                  threadId)
{
  const InputImageType * inputImage = this->GetInput();
  ...
  for (unsigned int loopCnt = 0; loopCnt < m_MaximumNumberOfIterations; ++loopCnt)
  {
    ...
    if (threadId == 0)
    {
      ...
      for (unsigned int i = 0; i < m_UpdateClusterPerThread.size(); ++i)
      {
        ...
        for (typename UpdateClusterMap::const_iterator clusterIter = clusterMap.begin();
             clusterIter != clusterMap.end();
             ++clusterIter)
        {
