...
public:
  ...
  TestIterator()
  {
    ...
    std::cout << "LargestRegion:" << region << '\n';
    if (TestLargestRegion() == false)
    {
      std::cout << "Failed testing largest region." << '\n';
      return false;
    }

    if (TestFrequenciesHaveHermitianSimmetry() == false)
    {
      std::cout << "Failed testing Hermitian simmetry." << '\n';
      return false;
    }

    region = m_PositiveHalfRegion;
    std::cout << "Positive Half Region (excluding 0 freqs in all dim):" << region << '\n';

    region = m_NegativeHalfRegion;
    std::cout << "Negative Half Region: (including Nyquist freq if N even)" << region << '\n';

    if (TestNegativeRegion(region) == false)
    {
      std::cout << "Failed testing negative frequencies region." << '\n';
      return false;
    }
      ...
      for (unsigned int dim = 0; dim < ImageType::ImageDimension; ++dim)
      {
        if (itk::Math::NotAlmostEquals(it.GetFrequency()[dim], itk::Math::abs(reverseIt.GetFrequency()[dim])))
        {
          std::cout << "Failed testing Hermitian property at index:" << it.GetIndex() << " freq: " << it.GetFrequency()
                    << '\n'
                    << " reverseIt: " << reverseIt.GetIndex() << " freq: " << reverseIt.GetFrequency() << '\n';
          return false;
        }
      }
      --reverseIt;
  ...
  TestLargestRegion()
  {
    ...
    while (!it.IsAtEnd())
    {
      ...
      {
        std::cerr << "Test failed! " << '\n';
        std::cerr << "Index is not inside region!: " << index << '\n';
        return false;
      }
      // Check repeatibility
      ...
      {
        std::cerr << "Test failed! " << '\n';
        std::cerr << "Failed to repeat GetIndex()" << '\n';
        return false;
      }
      ...
      {
        std::cerr << "Test failed! " << '\n';
        std::cerr << "Error in largest frequency bin" << '\n';
        std::cerr << "Expected: " << m_LargestFrequency << ", but got " << it.GetFrequency() << '\n';
        return false;
      }
...
itkFrequencyFFTLayoutImageRegionIteratorWithIndexTest(int, char *[])
{
  ...
  {
    const size_t inputImageSize(8);
    std::cout << "Testing with EVEN Image< std::complex<float>, 3 > with size: " << inputImageSize << '\n';
  ...
  // Even input image size test
  {
    const size_t inputImageSize(10);
    std::cout << "Testing with EVEN Image< char, 3 > with size: " << inputImageSize << '\n';
  ...
  {
    const size_t inputImageSize(9);
    std::cout << "Testing with ODD Image< char, 3 > with size: " << inputImageSize << '\n';
    itkFrequencyFFTLayoutImageRegionIteratorWithIndexTester<itk::Image<CharPixelType, Dimension>> Tester(
      inputImageSize);
    if (Tester.TestIterator() == false)
    {
