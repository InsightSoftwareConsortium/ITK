...
template <typename TInput, typename TOutput, typename TAuxValue, unsigned int VAuxDimension>
void
FastMarchingExtensionImageFilterBase<TInput, TOutput, TAuxValue, VAuxDimension>::GenerateOutputInformation()
{
...
template <typename TInput, typename TOutput, typename TAuxValue, unsigned int VAuxDimension>
void
FastMarchingExtensionImageFilterBase<TInput, TOutput, TAuxValue, VAuxDimension>::EnlargeOutputRequestedRegion(
...
template <typename TInput, typename TOutput, typename TAuxValue, unsigned int VAuxDimension>
void
FastMarchingExtensionImageFilterBase<TInput, TOutput, TAuxValue, VAuxDimension>::InitializeOutput(
  OutputImageType * oImage)
{
  ...
  {
    itkExceptionStringMacro("in Initialize(): Null pointer for AuxAliveValues");
  }
  if (m_AuxiliaryAliveValues->size() != (this->m_AlivePoints->size()))
  {
    itkExceptionStringMacro("in Initialize(): AuxAliveValues is the wrong size");
  }

  if (!m_AuxiliaryTrialValues)
  {
    itkExceptionStringMacro("in Initialize(): Null pointer for AuxTrialValues");
  }
  if (m_AuxiliaryTrialValues->size() != this->m_TrialPoints->size())
  {
    itkExceptionStringMacro("in Initialize(): AuxTrialValues is the wrong size");
  }
