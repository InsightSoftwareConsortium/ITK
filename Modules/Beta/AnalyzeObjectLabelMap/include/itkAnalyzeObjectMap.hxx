...
template <class TImage, class TRGBImage>
typename itk::AnalyzeObjectMap<TImage>::Pointer AnalyzeObjectMap<TImage, TRGBImage>::PickOneEntry(
  const int numberOfEntry)
{
...
void AnalyzeObjectMap<TImage, TRGBImage>::PlaceObjectMapEntriesIntoMetaData()
{
  ...
  itk::EncapsulateMetaData<itk::AnalyzeObjectEntryArrayType>(thisDic, ANALYZE_OBJECT_LABEL_MAP_ENTRY_ARRAY,
                                                             *my_reference);
}
...
::ImageToObjectMap(TImage *image)
{
  ...
  itk::AnalyzeObjectEntryArrayType *my_reference = this->GetAnalyzeObjectEntryArrayPointer();
  if( itk::ExposeMetaData<itk::AnalyzeObjectEntryArrayType>(image->GetMetaDataDictionary(),
                                                            ANALYZE_OBJECT_LABEL_MAP_ENTRY_ARRAY, *my_reference) )
    {
