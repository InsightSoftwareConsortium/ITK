...
int itkMultiScaleHessianEnhancementImageFilterTestStaticMethods( int, char * [] )
{
  ...
  /* Test that we get zero sized arrays for bad inputs */
  ArrayType expectedZeroSigmaArray;
  expectedZeroSigmaArray.SetSize(0);
  ...
  TEST_EXPECT_EQUAL(expectedZeroSigmaArray, sigmaArray);
...
int itkMultiScaleHessianEnhancementImageFilterTestStaticMethodGenerateLogarithmicSigmaArray( int, char * [] )
{
  ...
  TEST_EXPECT_EQUAL(expectedLogarithmicArray.GetSize(), logarithmicArray.GetSize())
  for (ArrayType::SizeValueType i = 0; i < expectedLogarithmicArray.GetSize(); ++i)
  {
    TEST_EXPECT_TRUE( itk::Math::FloatAlmostEqual(
      expectedLogarithmicArray.GetElement(i),
      logarithmicArray.GetElement(i),
      6,
      0.000001
    ));
  }
...
int itkMultiScaleHessianEnhancementImageFilterTestStaticMethodGenerateEquispacedSigmaArray( int, char * [] )
{
  ...
  /* Here we need to do some floating point comparisons. These are exact, but we should still do a floating point comparison to be sure */
  TEST_EXPECT_EQUAL(expectedEquidistanceArray.GetSize(), equidistanceArray.GetSize())
  for (ArrayType::SizeValueType i = 0; i < expectedEquidistanceArray.GetSize(); ++i)
  {
    TEST_EXPECT_TRUE( itk::Math::FloatAlmostEqual(
      expectedEquidistanceArray.GetElement(i),
      equidistanceArray.GetElement(i),
      6,
      0.000001
    ));
  }
