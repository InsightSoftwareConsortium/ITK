...
/**
  ...
  * \ingroup ITKCommon
  */
template<unsigned VDimension, bool VBeginAtZero = true>
...
public:
   ...
   * which case, it is removed by means of C++ SFINAE).
   */
  template <typename TDummy = typename std::enable_if<!VBeginAtZero, std::nullptr_t>::type>
  explicit IndexRange(const ImageRegion<VDimension>& imageRegion, TDummy = nullptr)
    :
    // Note: Use parentheses instead of curly braces to initialize data members,
    // to avoid AppleClang 6.0.0.6000056 compile errors, "no viable conversion..."
