...
{
  std::cout << "itkSymmetricEllipsoidInteriorExteriorSpatialFunction test start" << '\n';
  ...
  if (volumeError <= 5 || functionValue == 1)
  {
    ...
    std::cout << "calculated ellipsoid volume = " << volume << '\n'
              ...
              << "itkSymmetricEllipsoidSpatialFunction ended successfully!" << '\n';
    return EXIT_SUCCESS;
  }
  ...
  std::cerr << "calculated ellipsoid volume = " << volume << '\n'
            ...
            << "center location = (" << spatialFunc->GetCenter()[0] << ", " << spatialFunc->GetCenter()[0] << ", "
            << spatialFunc->GetCenter()[2] << ')' << '\n'
            << "itkSymmetricEllipsoidSpatialFunction failed :(" << '\n';
  return EXIT_FAILURE;
}
