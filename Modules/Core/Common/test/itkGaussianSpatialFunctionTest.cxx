...
itkGaussianSpatialFunctionTest(int argc, char * argv[])
{
  if (argc < 3)
  {
    ...
    std::cerr << "Usage: " << itkNameOfTestExecutableMacro(argv) << " scale normalized" << '\n';
    return EXIT_FAILURE;
  }
  ...
  if (itk::Math::NotAlmostEquals(expectedValueAtMean, computedValueAtMean))
  {
    ...
    std::cerr << "Error in Evaluate at point " << point << '\n';
    std::cerr << "Expected value " << expectedValueAtMean << '\n';
    std::cerr << " differs from " << computedValueAtMean << '\n';
    return EXIT_FAILURE;
  }
