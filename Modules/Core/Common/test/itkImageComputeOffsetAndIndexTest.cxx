...
ComputeOffset(TImage * image, unsigned int count, unsigned int repeat)
{
  ...
  for (unsigned int j = 0; j < repeat; ++j)
  {
    auto index = itk::MakeFilled<typename TImage::IndexType>(0);
    for (unsigned int i = 0; i < count; ++i)
    {
...
#define TRY_FAST_INDEX(dim)                                                       \
  {                                                                               \
    ...
    const auto                  size = itk::MakeFilled<ImageType::SizeType>(50);  \
    const auto                  index = itk::MakeFilled<ImageType::IndexType>(0); \
