...
bool
testValue(const double v1, const double v2, int maxUlps = 4)
{
...
bool
testMatrix(const TMatrix & m1, const TMatrix & m2, int maxUlps = 4)
{
  bool pass = true;
...
bool
testVector(const TVector & v1, const TVector & v2, int maxUlps = 4)
{
  bool pass = true;
...
itkAffineTransformTest(int, char *[])
{
  ...
  for (unsigned int i = 0; i < Affine3DType::MatrixType::RowDimensions; ++i)
  {
    for (unsigned int j = 0; j < Affine3DType::MatrixType::ColumnDimensions; ++j)
    {
      if (itk::Math::Absolute(jaffInverseJacobianPosition[i][j] - matrix3invTruth[i][j]) > 1e-13)
      {
  ...
  bool found_inv_inv_descrepancies = false;
  {
    ...
    for (unsigned int q = 0; q < parameters1_inv_inv.size(); ++q)
    {
      ...
      mag_error += sqrt(itk::Math::Absolute(v));
    }
    if (mag_error > 1e-4)
    {
