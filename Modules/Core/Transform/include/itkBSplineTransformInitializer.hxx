...
BSplineTransformInitializer<TTransform, TImage>::SetTransformDomainMeshSize(const MeshSizeType meshSize)
{
  ...
  if (this->m_SetTransformDomainMeshSizeViaInitializer == false || this->m_TransformDomainMeshSize != meshSize)
  {
...
template <typename TTransform, typename TImage>
void
BSplineTransformInitializer<TTransform, TImage>::InitializeTransform() const
{
  if (!this->m_Transform)
  {
  ...
  if (!this->m_Image)
  {
    itkExceptionStringMacro("Image has not been set.");
  }
  if (TImage::GetImageDimension() != SpaceDimension)
  {
    itkExceptionStringMacro("Image dimensionality does not match the transform.");
  }
...
template <typename TTransform, typename TImage>
void
BSplineTransformInitializer<TTransform, TImage>::PrintSelf(std::ostream & os, Indent indent) const
{
