...
::BeforeThreadedExecution()
{
  ...
  /* Porting: these next blocks of code are from
    MattesMutualImageToImageMetric::Initialize */
  ...
  if( this->m_MattesAssociate->m_ThreaderFixedImageMarginalPDF.size() != mattesAssociateNumThreadsUsed )
    {
                ...
                const ThreadIdType                 threadId) const
{
  ...
  const OffsetValueType fixedImageParzenWindowIndex =
    this->m_MattesAssociate->ComputeSingleFixedImageParzenWindowIndex(
      fixedImageValue );
  ...
  const OffsetValueType pdfMovingParzenWindowIndexStart =
    static_cast<OffsetValueType>( movingImageParzenWindowIndex ) - 1;
  PDFValueType movingImageParzenWindowArg = static_cast<PDFValueType>( pdfMovingParzenWindowIndexStart )
  ...
  // Just store the starting pdfMovingParzenWindowIndexStart and we'll iterate
  // later over the next four to collect results.
  if( doComputeDerivative && ( this->m_MattesAssociate->HasLocalSupport() ) )
    {
    const OffsetValueType jointPdfIndex1D = pdfMovingParzenWindowIndexStart
      + (fixedImageParzenWindowIndex * this->m_MattesAssociate->m_NumberOfHistogramBins);
    localDerivativeOffset = this->m_MattesAssociate->ComputeParameterOffsetFromVirtualIndex( virtualIndex,
                                                                                             this->GetCachedNumberOfLocalParameters() );
  ...
  JointPDFValueType *pdfPtr = this->m_MattesAssociate->m_ThreaderJointPDF[threadId]->GetBufferPointer()
    + ( fixedImageParzenWindowIndex
        * this->m_MattesAssociate->m_NumberOfHistogramBins ) + pdfMovingParzenWindowIndexStart;
  ...
  const OffsetValueType pdfMovingParzenWindowIndexEnd = static_cast<OffsetValueType>( movingImageParzenWindowIndex )
    + 2;
  OffsetValueType pdfMovingParzenWindowIndex = pdfMovingParzenWindowIndexStart;

  while( pdfMovingParzenWindowIndex <= pdfMovingParzenWindowIndexEnd )
    {
      {
    ...
    if( doComputeDerivative )
      ...
      if( transformIsDisplacement )
        ...
        const OffsetValueType ThisIndexOffset =
          ...
          + ( pdfMovingParzenWindowIndex * this->m_MattesAssociate->m_JointPDFDerivatives->GetOffsetTable()[1] );
        ...
        this->m_MattesAssociate->m_ThreaderDerivativeManager[threadId].CheckAndDumpIfNecessary();
        }
      }

    movingImageParzenWindowArg += 1.0;
    ++pdfMovingParzenWindowIndex;
    ++movingParzenBin;
    }
  ...
  DerivativeValueType *           localSupportDerivativeResultPtr) const
{
  for( NumberOfParametersType mu = 0, maxElement = this->GetCachedNumberOfLocalParameters(); mu < maxElement; ++mu )
    ...
    const PDFValueType derivativeLocalSupportContribution = innerProduct * cubicBSplineDerivativeValue;
    *( localSupportDerivativeResultPtr ) += derivativeLocalSupportContribution;
    localSupportDerivativeResultPtr++;
    }
}
