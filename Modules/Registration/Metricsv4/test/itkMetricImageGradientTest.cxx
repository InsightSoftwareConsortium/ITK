                                  ...
                                  std::string &                outputPath)
{
  ...
  it.GoToBegin();
  static constexpr unsigned int imageBorder{ 20 };
  while (!it.IsAtEnd())
  {
    it.Set(0);
  ...
  static constexpr bool writeImages{ false };
...
itkMetricImageGradientTest(int argc, char * argv[])
{
  ...
  static constexpr DimensionSizeType imageSize{ 60 };
  unsigned int                       dimensionality = 3;
  double                             minimumAverage = itk::NumericTraits<double>::max();
  double                             rotationDegrees = 0.0; // (3.0);
  static constexpr double            maxDegrees{ 359.0 };
  static constexpr double            degreeStep{ 15.0 }; //(3.0);
  ...
  for (dimensionality = 2; dimensionality <= 3; ++dimensionality)
  {
    ...
    static constexpr double threshold{ 0.96 };
