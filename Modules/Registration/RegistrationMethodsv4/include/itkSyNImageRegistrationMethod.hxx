               ...
               ObjectToObjectMetricBaseTemplateEnums::MetricCategory::IMAGE_METRIC)
      {
        if (!this->m_DownsampleImagesForMetricDerivatives)
        {
          ...
          dynamic_cast<ImageMetricType *>(multiMetric->GetMetricQueue()[n].GetPointer())
            ->SetFixedImageMask(fixedImageMasks[n]);
          dynamic_cast<ImageMetricType *>(multiMetric->GetMetricQueue()[n].GetPointer())
            ->SetMovingImageMask(movingImageMasks[n]);
        }
        else
        {
          ...
          if (fixedImageMasks[n])
          {
            ...
            dynamic_cast<ImageMetricType *>(multiMetric->GetMetricQueue()[n].GetPointer())
              ->SetFixedImageMask(resampledFixedImageMask);
          }
  ...
  else
  {
    if (this->m_Metric->GetMetricCategory() == ObjectToObjectMetricBaseTemplateEnums::MetricCategory::POINT_SET_METRIC)
    {
      ...
      dynamic_cast<PointSetMetricType *>(this->m_Metric.GetPointer())
        ->SetFixedTransform(const_cast<TransformBaseType *>(fixedTransform));
      dynamic_cast<PointSetMetricType *>(this->m_Metric.GetPointer())
        ->SetMovingTransform(const_cast<TransformBaseType *>(movingTransform));
...
template <typename TFixedImage,
          ...
          typename TPointSet>
void
SyNImageRegistrationMethod<TFixedImage, TMovingImage, TOutputTransform, TVirtualImage, TPointSet>::GenerateData()
{
  ...
  Indent         indent) const
{
  ...
  os << indent << "ConvergenceThreshold: "
     << static_cast<typename NumericTraits<RealType>::PrintType>(this->m_ConvergenceThreshold) << '\n';
  os << indent << "ConvergenceWindowSize: " << this->m_ConvergenceWindowSize << '\n';

  itkPrintSelfObjectMacro(MovingToMiddleTransform);
  itkPrintSelfObjectMacro(FixedToMiddleTransform);

  os << indent << "NumberOfIterationsPerLevel: " << this->m_NumberOfIterationsPerLevel << '\n';
  os << indent << "DownsampleImagesForMetricDerivatives: " << m_DownsampleImagesForMetricDerivatives << '\n';
  os << indent << "AverageMidPointGradients: " << m_AverageMidPointGradients << '\n';
