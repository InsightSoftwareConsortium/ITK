...
#  endif
  ...
  m_PlanForwardDCT = fftw_plan_r2r(ImageDimension,
                                   fixedImageDimensionsFFTW,
  ...
  m_PlanBackwardDCT = fftw_plan_r2r(ImageDimension,
                                    fixedImageDimensionsFFTW,
                                    m_DisplacementFieldComponentImageDCT,
                                    m_DisplacementFieldComponentImage,
                                    fftBackward,
...
#  endif
  ...
  for (unsigned int l = 0; l < DeformationVectorDimension; ++l)
  {
    ...
    for (SizeValueType offset1 = 0; (offset1 < numberOfPixels) && !itInDeformation.IsAtEnd() && !itInUpdate.IsAtEnd();
         ++offset1)
    {
      this->m_DisplacementFieldComponentImage[offset1] =
        this->m_TimeStep * itInUpdate.Value()[l] + itInDeformation.Value()[l];
      ++itInUpdate;
      ++itInDeformation;
    }
