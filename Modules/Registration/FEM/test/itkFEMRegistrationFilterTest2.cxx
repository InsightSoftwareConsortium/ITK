...
int itkFEMRegistrationFilterTest2(int argc, char *argv[] )
{
  ...
  for( unsigned int met = 0; met < 4; met++ )
    ...
    registrator->SetElasticity(e, 1);
    registrator->SetRho(p, 0);
    registrator->SetRho(p, 1);
    registrator->SetGamma(1., 0);
    registrator->SetGamma(1., 1);
    registrator->SetAlpha(1.);
    registrator->SetMaximumIterations( maxiters, 0 );
    registrator->SetMaximumIterations( maxiters, 1 );
    ...
    registrator->SetUseLandmarks( true );
