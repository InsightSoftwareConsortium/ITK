  ...
  FEMVectorType Gsol)
{
  ...
  for ( k = 0; k < ImageDimension; k++ )
    ...
    else { oindex[k] = (OIndexValueType)( Gpos[k] + 0.5 ); }
...
{
  std::string clsnm = std::string("FiniteDifferenceFunctionLoad(") + typeid( TMoving ).name() + ","
                      + typeid( TFixed ).name() + ")";
...
template< class TMoving, class TFixed >
const int FiniteDifferenceFunctionLoad< TMoving,
                                        TFixed >::m_DummyCLID = FiniteDifferenceFunctionLoad< TMoving, TFixed >::CLID();
} // end namespace fem
} // end namespace itk
