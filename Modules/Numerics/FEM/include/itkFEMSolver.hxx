...
Solver<VDimension>::SetInput(FEMObjectType * fem)
{
  ...
  this->ProcessObject::SetNthInput(0, const_cast<FEMObjectType *>(fem));
...
{
  // Process object is not const-correct so the const_cast is required here
  this->ProcessObject::SetNthInput(index, const_cast<FEMObjectType *>(fem));
  this->m_FEMObject = fem;
  this->m_NGFN = fem->GetNumberOfDegreesOfFreedom();
  this->m_NMFC = fem->GetNumberOfMultiFreedomConstraints();
}
...
template <unsigned int VDimension>
void
Solver<VDimension>::GenerateData()
{
  // Call Solver
  this->RunSolver();
}
...
template <unsigned int VDimension>
void
Solver<VDimension>::SetLinearSystemWrapper(LinearSystemWrapper::Pointer ls)
{
...
template <unsigned int VDimension>
void
Solver<VDimension>::InitializeLinearSystemWrapper()
{
...
template <unsigned int VDimension>
void
Solver<VDimension>::InitializeMatrixForAssembly(unsigned int N)
{
...
template <unsigned int VDimension>
void
Solver<VDimension>::AssembleLandmarkContribution(Element::ConstPointer e, float eta)
{
  ...
  for (int j = 0; j < Ne; ++j)
  {
    ...
    for (int k = 0; k < Ne; ++k)
    {
      ...
      if (e->GetDegreeOfFreedom(j) >= this->m_NGFN || e->GetDegreeOfFreedom(k) >= this->m_NGFN)
      {
...
template <unsigned int VDimension>
void
Solver<VDimension>::AssembleElementMatrix(Element::Pointer e)
{
...
template <unsigned int VDimension>
void
Solver<VDimension>::AssembleF(int dim)
{
  ...
  for (unsigned int l = 0; l < numberOfLoads; ++l)
  {
    ...
    if (LoadNode::Pointer l1 = dynamic_cast<LoadNode *>(l0.GetPointer()))
    {
      ...
      {
        throw FEMExceptionSolution(
          __FILE__, __LINE__, "Solver::AssembleF()", "Illegal size of a force vector in LoadNode object!");
      }
      ...
      for (unsigned int d = 0; d < (l1->GetElement()->GetNumberOfDegreesOfFreedomPerNode()); ++d)
      {
        ...
        // dimensions
        m_LinearSystem->AddVectorValue(
          dof, l1->GetForce()[d + l1->GetElement()->GetNumberOfDegreesOfFreedomPerNode() * dim]);
      }
...
Solver<VDimension>::RunSolver()
{
  ...
  {
    throw FEMExceptionSolution(
      __FILE__, __LINE__, "FEMObject::Solve()", "Master stiffness matrix was not initialized!");
  }
...
template <unsigned int VDimension>
void
Solver<VDimension>::UpdateDisplacements()
{
...
Solver<VDimension>::ApplyBC(int dim, unsigned int matrix)
{
  ...
  for (unsigned int i = 0; i < numberOfLoads; ++i)
  {
    ...
    if (LoadBCMFC::Pointer c = dynamic_cast<LoadBCMFC *>(l0.GetPointer()))
    {
      ...
      for (auto q = c->GetLeftHandSideArray().begin(); q != c->GetLeftHandSideArray().end(); ++q)
      {
...
template <unsigned int VDimension>
void
Solver<VDimension>::FillInterpolationGrid()
{
  VectorType v1, v2;
