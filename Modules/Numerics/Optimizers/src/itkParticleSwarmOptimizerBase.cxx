...
ParticleSwarmOptimizerBase::SetNumberOfParticles(unsigned int n)
{
  ...
  {
    itkExceptionStringMacro("swarm already set with different size, clear swarm and then set");
  }
...
ParticleSwarmOptimizerBase::SetInitialSwarm(const SwarmType & initialSwarm)
{
  ...
  if (!initialSwarm.empty())
  {
    ...
    for (auto it = initialSwarm.begin(); it != initialSwarm_END; ++it)
    {
      ...
      {
        itkExceptionStringMacro("inconsistent dimensions in swarm data");
      }
    }
...
ParticleSwarmOptimizerBase::StartOptimization()
{
  ...
  for (this->m_IterationIndex = 1; m_IterationIndex < m_MaximalNumberOfIterations && !converged; ++m_IterationIndex)
  {
    ...
    for (unsigned int j = 0; j < this->m_NumberOfParticles; ++j)
    {
    ...
    {
      const unsigned int prevIndex = (index == this->m_NumberOfGenerationsWithMinimalImprovement) ? 0 : index + 1;
          ...
          this->m_FunctionConvergenceTolerance)
      {
        ...
        for (unsigned int k = 0; k < n && converged; ++k)
        {
          for (unsigned int j = 0; j < this->m_NumberOfParticles; ++j)
          {
            parameterDiffs[j] =
              itk::Math::Absolute(this->m_Particles[j].m_BestParameters[k] - this->m_ParametersBestValue[k]);
          }
...
ParticleSwarmOptimizerBase::ValidateSettings()
{
  ...
  {
    itkExceptionStringMacro("nullptr cost function");
  }
  ...
  {
    itkExceptionStringMacro("cost function and initial position dimensions mismatch");
  }
  // at least one particle
  if (this->m_NumberOfParticles == 0)
  {
    itkExceptionStringMacro("number of particles is zero");
  }
  // at least one iteration (the initialization phase)
  if (this->m_MaximalNumberOfIterations == 0)
  {
    itkExceptionStringMacro("number of iterations is zero");
  }
  // we need at least one generation difference to
  // compare to the previous one
  if (this->m_NumberOfGenerationsWithMinimalImprovement == 0)
  {
    itkExceptionStringMacro("number of generations with minimal improvement is zero");
  }

  if (this->m_ParameterBounds.size() != n)
  {
    itkExceptionStringMacro("cost function and parameter bounds dimensions mismatch");
  }
  for (unsigned int i = 0; i < n; ++i)
  {
    if (initialPosition[i] < this->m_ParameterBounds[i].first || initialPosition[i] > this->m_ParameterBounds[i].second)
    {
      itkExceptionStringMacro("initial position is outside specified parameter bounds");
    }
  }
  ...
  if (!this->m_Particles.empty())
  {
    ...
    {
      itkExceptionStringMacro("cost function and particle data dimensions mismatch");
    }
    const auto end = this->m_Particles.end();
    for (auto it = this->m_Particles.begin(); it != end; ++it)
    {
  ...
  for (unsigned int i = 0; i < n; ++i)
  {
    ...
    {
      itkExceptionStringMacro("negative parameters convergence tolerance");
    }
  }
  // function convergence tolerance has to be positive
  if (this->m_FunctionConvergenceTolerance < 0)
  {
    itkExceptionStringMacro("negative function convergence tolerance");
  }
}
...
ParticleSwarmOptimizerBase::RandomInitialization()
{
  ...
  for (unsigned int i = 0; i < this->m_NumberOfParticles; ++i)
  {
  ...
  if (this->m_InitializeNormalDistribution)
  {
    ...
    for (unsigned int i = 1; i < this->m_NumberOfParticles; ++i)
    {
