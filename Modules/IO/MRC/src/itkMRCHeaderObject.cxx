...
MRCHeaderObject::SetHeader(const Header * buffer)
{
  ...
  else if (_header->stamp[0] == 0 && _header->stamp[1] == 0 && _header->stamp[2] == 0 && _header->stamp[3] == 0)
  {
    this->m_BigEndianHeader = ByteSwapper<void *>::SystemIsBigEndian();

    // do a check to see which byte order make sense
    if (this->m_Header.mapc < 1 || this->m_Header.mapc > 3 || this->m_Header.mapr < 1 || this->m_Header.mapr > 3 ||
...
SizeValueType
MRCHeaderObject::GetExtendedHeaderSize() const
{
...
bool
MRCHeaderObject::IsOriginalHeaderBigEndian() const
{
