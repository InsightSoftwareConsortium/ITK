...
itkScancoImageIOTest2(int argc, char * argv[])
{
  ...
  ITK_TEST_EXPECT_EQUAL(scancoIO->GetPatientIndex(), 1);
  int intMeta;
  itk::ExposeMetaData<int>(metaData, "PatientIndex", intMeta);
  ITK_TEST_EXPECT_EQUAL(intMeta, 1);
  ...
  ITK_TEST_EXPECT_EQUAL(intMeta, 3401);
  std::cout << "SliceThickness: \t" << scancoIO->GetSliceThickness() << std::endl;
  ITK_TEST_EXPECT_TRUE(itk::Math::FloatAlmostEqual(scancoIO->GetSliceThickness(), 0.12138, 6, 1e-3));
  double doubleMeta;
  itk::ExposeMetaData<double>(metaData, "SliceThickness", doubleMeta);
  ITK_TEST_EXPECT_TRUE(itk::Math::FloatAlmostEqual(doubleMeta, 0.12138, 6, 1e-3));
  std::cout << "SliceIncrement: \t" << scancoIO->GetSliceIncrement() << std::endl;
  ITK_TEST_EXPECT_TRUE(itk::Math::FloatAlmostEqual(scancoIO->GetSliceIncrement(), 0.12138, 6, 1e-3));
  itk::ExposeMetaData<double>(metaData, "SliceIncrement", doubleMeta);
  ITK_TEST_EXPECT_TRUE(itk::Math::FloatAlmostEqual(doubleMeta, 0.12138, 6, 1e-3));
  std::cout << "StartPosition: \t" << scancoIO->GetStartPosition() << std::endl;
  ITK_TEST_EXPECT_TRUE(itk::Math::FloatAlmostEqual(scancoIO->GetStartPosition(), 150.0, 6, 1e-3));
  ...
  ITK_TEST_EXPECT_TRUE(itk::Math::FloatAlmostEqual(scancoIO->GetDataRange()[0], 953.0, 6, 1e-3));
  ITK_TEST_EXPECT_TRUE(itk::Math::FloatAlmostEqual(vectorDoubleMeta[0], 953.0, 6, 1e-3));
  std::cout << "DataRange[1]: \t" << scancoIO->GetDataRange()[1] << std::endl;
  ITK_TEST_EXPECT_TRUE(itk::Math::FloatAlmostEqual(scancoIO->GetDataRange()[1], 7707.0, 6, 1e-3));
  ITK_TEST_EXPECT_TRUE(itk::Math::FloatAlmostEqual(vectorDoubleMeta[1], 7707.0, 6, 1e-3));
  ...
  ITK_TEST_EXPECT_TRUE(itk::Math::FloatAlmostEqual(doubleMeta, 8192.0, 6, 1e-3));
  ...
  ITK_TEST_EXPECT_EQUAL(scancoIO->GetRescaleType(), 0);
  itk::ExposeMetaData<int>(metaData, "RescaleType", intMeta);
  ITK_TEST_EXPECT_EQUAL(intMeta, 0);
  std::cout << "RescaleSlope: \t\t" << scancoIO->GetRescaleSlope() << std::endl;
  ITK_TEST_EXPECT_TRUE(itk::Math::FloatAlmostEqual(scancoIO->GetRescaleSlope(), 1.0, 6, 1e-3));
  itk::ExposeMetaData<double>(metaData, "RescaleSlope", doubleMeta);
  ITK_TEST_EXPECT_TRUE(itk::Math::FloatAlmostEqual(doubleMeta, 1.0, 6, 1e-3));
  std::cout << "RescaleIntercept: \t\t" << scancoIO->GetRescaleIntercept() << std::endl;
  ITK_TEST_EXPECT_TRUE(itk::Math::FloatAlmostEqual(scancoIO->GetRescaleIntercept(), 0.0, 6, 1e-3));
  itk::ExposeMetaData<double>(metaData, "RescaleIntercept", doubleMeta);
  ITK_TEST_EXPECT_TRUE(itk::Math::FloatAlmostEqual(doubleMeta, 0.0, 6, 1e-3));
  std::cout << "RescaleUnits: \t\t" << scancoIO->GetRescaleUnits() << std::endl;
  ITK_TEST_EXPECT_EQUAL(scancoIO->GetRescaleUnits(), std::string(""));
  itk::ExposeMetaData<std::string>(metaData, "RescaleUnits", stringMeta);
  ITK_TEST_EXPECT_EQUAL(stringMeta, std::string(""));
  std::cout << "CalibrationData: \t\t" << scancoIO->GetCalibrationData() << std::endl;
  ITK_TEST_EXPECT_EQUAL(scancoIO->GetCalibrationData(), std::string(""));
  ...
  ITK_TEST_EXPECT_EQUAL(scancoIO->GetNumberOfSamples(), 1150);
  itk::ExposeMetaData<int>(metaData, "NumberOfSamples", intMeta);
  ITK_TEST_EXPECT_EQUAL(intMeta, 1150);
  std::cout << "NumberOfProjections: " << scancoIO->GetNumberOfProjections() << std::endl;
  ITK_TEST_EXPECT_EQUAL(scancoIO->GetNumberOfProjections(), 600);
  itk::ExposeMetaData<int>(metaData, "NumberOfProjections", intMeta);
  ITK_TEST_EXPECT_EQUAL(intMeta, 600);
  std::cout << "ScanDistance: \t" << scancoIO->GetScanDistance() << std::endl;
  ITK_TEST_EXPECT_TRUE(itk::Math::FloatAlmostEqual(scancoIO->GetScanDistance(), 139.61, 6, 1e-3));
  itk::ExposeMetaData<double>(metaData, "ScanDistance", doubleMeta);
  ITK_TEST_EXPECT_TRUE(itk::Math::FloatAlmostEqual(doubleMeta, 139.61, 6, 1e-3));
  ...
  ITK_TEST_EXPECT_EQUAL(intMeta, 9);
  ...
  ITK_TEST_EXPECT_TRUE(itk::Math::FloatAlmostEqual(doubleMeta, 43.0, 6, 1e-3));
  std::cout << "MeasurementIndex: \t" << scancoIO->GetMeasurementIndex() << std::endl;
  ITK_TEST_EXPECT_EQUAL(scancoIO->GetMeasurementIndex(), 26153);
  itk::ExposeMetaData<int>(metaData, "MeasurementIndex", intMeta);
  ITK_TEST_EXPECT_EQUAL(intMeta, 26153);
  std::cout << "Site: \t\t" << scancoIO->GetSite() << std::endl;
  ITK_TEST_EXPECT_EQUAL(scancoIO->GetSite(), 4);
  itk::ExposeMetaData<int>(metaData, "Site", intMeta);
  ITK_TEST_EXPECT_EQUAL(intMeta, 4);
  ...
  ITK_TEST_EXPECT_TRUE(itk::Math::FloatAlmostEqual(doubleMeta, 68.0, 6, 1e-3));
  std::cout << "Intensity: \t\t" << scancoIO->GetIntensity() << std::endl;
  ITK_TEST_EXPECT_TRUE(itk::Math::FloatAlmostEqual(scancoIO->GetIntensity(), 1.47, 6, 1e-3));
  itk::ExposeMetaData<double>(metaData, "Intensity", doubleMeta);
  ITK_TEST_EXPECT_TRUE(itk::Math::FloatAlmostEqual(doubleMeta, 1.47, 6, 1e-3));
