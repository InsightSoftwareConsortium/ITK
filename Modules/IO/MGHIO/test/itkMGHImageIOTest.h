...
itkMGHImageIOTestGenerateRandomImage<itk::DiffusionTensor3D<float>, 3>(unsigned int size)
{
  ...
  itk::ImageRegionIteratorWithIndex<TensorImageType> tensorIt(tensorImage, tensorImage->GetLargestPossibleRegion());
...
template <class TPixelType, unsigned int VImageDimension>
bool
itkMGHImageIOTestReadWriteTest(std::string fn, unsigned int size, std::string inputFile, bool compression = false)
{
  using ImageType = itk::Image<TPixelType, VImageDimension>;
  ...
  for (int idx = 0; idx < 3; ++idx)
  {
    ...
    if (!itk::Math::FloatAlmostEqual(test_image->GetSpacing()[idx], reference_image->GetSpacing()[idx], 4, 1e-7))
    {
      isFailingSpacing = true;
    }
    if (!itk::Math::FloatAlmostEqual(test_image->GetOrigin()[idx], reference_image->GetOrigin()[idx], 100000, 1e-1))
    {
    ...
    for (int idx2 = 0; idx2 < 3; ++idx2)
    {
      if (!itk::Math::FloatAlmostEqual(
            test_image->GetDirection()[idx][idx2], reference_image->GetDirection()[idx][idx2], 4, 1e-7))
      {
  ...
  if (isFailingSpacing)
  {
    std::cerr << "ERROR:  Invalid Spacing: " << test_image->GetSpacing() << " ! = " << reference_image->GetSpacing()
              << std::endl;
  }
  if (isFailingOrigin)
  {
    std::cerr << "ERROR:  Invalid Origin: " << test_image->GetOrigin() << " ! = " << reference_image->GetOrigin()
              << std::endl;
  }
  if (isFailingDirection)
  {
    std::cerr << "ERROR:  Invalid Direction: \n"
              ...
              << reference_image->GetDirection() << std::endl;
  }
