                ...
                TEqual &&           pixelEqual)
{
  ...
  {
    std::cerr << label << " FAILED: region mismatch " << vti->GetLargestPossibleRegion() << " vs "
  ...
  for (vIt.GoToBegin(), mIt.GoToBegin(); !vIt.IsAtEnd(); ++vIt, ++mIt)
  {
    ...
    {
      std::cerr << label << " FAILED: pixel mismatch at " << vIt.GetIndex() << ": vti=" << vIt.Get()
                << " mhd=" << mIt.Get() << std::endl;
      return EXIT_FAILURE;
    }
  }
  std::cout << label << " OK: " << vtiPath << " matches " << mhdPath << std::endl;
  return EXIT_SUCCESS;
}
