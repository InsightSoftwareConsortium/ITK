         ...
         JDIMENSION width)
{
  ...
  scaler_ptr scalerv = (scaler_ptr) losslsd->scaler_private;
  int scale_factor = scalerv->scale_factor;
  unsigned int xindex;
...
scaler_start_pass (j_decompress_ptr cinfo)
{
  ...
  scalerv->scale_factor = cinfo->Al - downscale;

  /* Set scaler functions based on scale_factor (positive = left shift) */
  if (scalerv->scale_factor > 0)
    losslsd->scaler_scale = simple_upscale;
  else if (scalerv->scale_factor < 0) {
    scalerv->scale_factor = -scalerv->scale_factor;
...
jinit_d_scaler (j_decompress_ptr cinfo)
{
  ...
  scalerv = (scaler_ptr)
    (*cinfo->mem->alloc_small) ((j_common_ptr) cinfo, JPOOL_IMAGE,
        SIZEOF(scalerv));
  losslsd->scaler_private = (void *) scalerv;
