...
unsigned int compare_known_values(nifti_image const * const known, const nifti_image const * const unknown)
{
...
int main (int argc, char *argv[])
{
  ...
  {
    int KnownValid=nifti_validfilename("ValidFilename.img.gz");
    if(KnownValid != 1) { return __LINE__ ; }
  }
  {
    int KnownInvalid=nifti_validfilename("InvalidFilename.tiff");
    if(KnownInvalid != 0) { return __LINE__ ; }
  }
  {
    int NoPrefix=nifti_validfilename(".nii");
    if(NoPrefix != 0) { return __LINE__ ; }
  }
  {
    char * basename=nifti_makebasename("thebasename.nii");
    printf("thebasename=%s\n",basename);
    if(strcmp(basename,"thebasename") != 0) { return __LINE__; }
    free(basename);
    fflush(stdout);
  }
return 0;
