...
def create_enum_class(housing_class,enum_class,enum_values):
    ...
    output += "//Define how to print enumeration\nextern ITKCommon_EXPORT std::ostream &operator<<(std::ostream & out, const "+name+" value);\n\n"
    output+="/** Print enum values */\nstd::ostream &\noperator<<(std::ostream & out, const "+name+" value)\n{\n\treturn out << [value] {\n\t\tswitch (value)\n\t\t{"
    for value in enum_values:
        output+="\n\t\t\tcase "+name+"::"+value+":\n\t\t\t\treturn \"itk::"+name+"::"+value+"\";"
    output+="\n\t\t\tdefault: \n\t\t\t\treturn \"INVALID VALUE FOR itk::"+name+"\";\n\t\t}\n\t}();\n}\n\n"

    #Create test function
    output+= "//Test streaming enumeration for "+name+" elements\n const std::set<itk::" + name + "> all" + name + "{\n"
    for value in enum_values:
        output+="\titk::"+name+"::"+value+",\n"
    output= output[:-2] + "};\n for(const auto & ee : all" + name + ")\n{\n\tstd::cout << \"STREAMED ENUM VALUE "+name+": \" << ee << std::endl;\n}\n"
...
if __name__ == '__main__':
    print("This will create a basic strongly typed enum class following the new ITKv5 migration guide.\nNOTE: Some refactoring will likely be needed.")
    class_containing_enum_class = input("Enter class enum originally belongs to: ") + "Enums"
    enum_class = input("Enter the name of the enum class (should not contain enum at the end): ")
