...
cat > ${CMTMSG} << EOF
PERF: Provide performance enhancements

SRCDIR=${SRCDIR} #My local SRC
BLDDIR=${BLDDIR} #My local BLD

cd ${BLDDIR}
run-clang-tidy.py -checks=-*,performance*  -header-filter=.* -fix
...
cd ${BLDDIR}
/Users/johnsonhj/Dashboard/src/scripts/run-clang-tidy.py -checks=-*,modernize-use-override  -header-filter=.* -fix
