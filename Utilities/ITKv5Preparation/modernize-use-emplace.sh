...
cd ${BLDDIR}
run-clang-tidy.py -checks=-*,modernize-use-emplace  -header-filter=.* -fix
...
cd ${BLDDIR}
/Users/johnsonhj/Dashboard/src/scripts/run-clang-tidy.py -checks=-*,modernize-use-emplace  -header-filter=.* -fix
